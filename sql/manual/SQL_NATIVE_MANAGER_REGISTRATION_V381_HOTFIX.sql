-- V38.1: hotfix ثبت‌نام مدیر برای profile موقت role=teacher ساخته‌شده توسط trigger قدیمی.
-- هیچ حساب معلم دارای داده یا عضویت مدرسه تبدیل نمی‌شود.
begin;
...
begin
    ...
    -- برخی triggerهای قدیمی profile حساب ایمیلی تازه را پیش از setup با role=teacher
    -- می‌سازند. تبدیل چنین profile خالی مجاز است، اما حساب دانش‌آموز مدیریت‌شده یا
    -- معلمی که داده/عضویت واقعی دارد هرگز به مدیر تبدیل نمی‌شود.
    if v_profile.teacher_id is not null then
        return jsonb_build_object('error', 'این حساب دانش‌آموزی تحت مدیریت یک معلم است');
    end if;
    if v_profile.role not in ('student','teacher','manager') then
        return jsonb_build_object('error', 'نقش فعلی حساب قابل تبدیل به مدیر/معاون نیست');
    end if;
    if v_profile.role = 'teacher' and (
        exists (select 1 from public.classes c where c.teacher_id = v_uid)
        ...
        return jsonb_build_object('error', 'این ایمیل قبلاً حساب معلم فعال دارد و قابل تبدیل نیست');
    end if;
