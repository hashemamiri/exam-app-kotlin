-- V135.6 — رفع «این حساب قابل تبدیل به مدیر/معاون نیست» هنگام ثبت‌نام مدیر.
-- ریشه: trigger قدیمی وب‌اپ، profile حساب ایمیلی تازه را با role='teacher' می‌سازد؛
-- تابع مدیر فقط role in ('student','manager') را می‌پذیرفت و حساب تازهٔ «مدیر» رد می‌شد.
-- مسیر معلم در V60.3 همین «معلمِ خالی» را می‌پذیرفت؛ همان قاعده به مسیر مدیر اضافه شد.
-- بقیهٔ تابع عیناً V75.1 است (گاردهای تأیید ایمیل و نقش انتخابی حفظ شده‌اند).
...
begin
    ...
    -- V135.6 — trigger قدیمی وب‌اپ هر حساب ایمیلی تازه را با role='teacher' می‌سازد؛
    -- «معلمِ خالی» (بدون نام کاربری/کلاس/آزمون/دانش‌آموز/عضویت) که در ثبت‌نام «مدیر»
    -- را انتخاب کرده باید بتواند مدیر شود (همان قاعدهٔ V60.3 برای مسیر معلم).
    if v_profile.role = 'teacher' then
        if coalesce(v_profile.username, '') <> ''
           or exists (select 1 from public.classes c where c.teacher_id = v_uid)
           or exists (select 1 from public.exams e where e.teacher_id = v_uid)
           or exists (select 1 from public.profiles s where s.teacher_id = v_uid and s.role = 'student')
           or exists (select 1 from public.school_memberships sm where sm.user_id = v_uid and sm.status = 'active') then
            return jsonb_build_object('error', 'این حساب معلم فعال است و قابل تبدیل به مدیر/معاون نیست');
        end if;
    elsif v_profile.teacher_id is not null or v_profile.role not in ('student','manager') then
