...
# V131 — انتخابگرهای بومی پنل 📐 (کاغذ/جهت/حاشیه/فاصله)، محو نوار قالب‌بندی، کارت «کارنامه»، عمق سایهٔ مقیاس‌پذیر
_wh = read(WEB_ENGINE / "webhost.js")
require("b.pickOption(id, sel.value" in _wh and "setOption: setOption" in _wh and "function hookSetupToggle()" in _wh and "function bindFmtScrollHide(bar)" in _wh, "native option pickers / fmt bar hide missing")
require("fun pickOption(id: String?, current: String?, optionsJson: String?)" in dialog and "internal fun PrintOptionPickerDialog(" in dialog, "PrintOptionPickerDialog missing")
_pe0 = read(WEB_ENGINE / "pgs_engine.js")
require("<label>عرض (mm):</label>" in _pe0 and "<label>عرض سفارشی (mm):</label>" not in _pe0 and "__pgsSyncCustomState" in _pe0, "custom W/H labels")
_cards = read(ROOT / "app/src/main/java/ir/exam/app/ui/app/TeacherManagementCardsScreen.kt")
require("const val CARD_COUNT = 7" in _cards and '"کارنامه"' in _cards and "onGradeList" in _cards, "grade-list card missing")
require("fun neoDepth(base: Dp): Dp" in read(ROOT / "app/src/main/java/ir/exam/app/ui/app/Neumorphic69Design.kt"), "neoDepth missing")
# V132 — کادرِ چسبیده و دستگیره‌های لمس‌پذیر، هزینهٔ چاپ، کارت «سایت»، آیکن‌های کارت آزمون، گاردِ پنل
_wc = read(WEB_ENGINE / "webhost.css")
require("#pgsViewer #previewArea .interactive-figure .qmf-fig{padding:2px !important" in _wc, "V132 figure box must hug the object")
require(".fig-resize-handle::before{content:\"\";position:absolute;left:50%;top:50%;width:44px;height:44px" in _wc, "V132 handle touch slop missing")
...
require("private fun ExamCardAction(" in read(MAIN / "java/ir/exam/app/ui/dashboard/TeacherDashboardScreen.kt"), "V132 exam card icon actions missing")
require("private suspend fun guardPanelRole(user: AppUser, pane: String?)" in read(MAIN / "java/ir/exam/app/ui/auth/AuthViewModel.kt"), "V132 cross-role login guard missing")
