...
class NativeMathCanvasRenderer {
 fun measure(node:MathNode,size:Float):MathCanvasSize=when(node){
  ...
  is MathNode.Radical->{val b=measure(node.body,size);MathCanvasSize(b.width+size*.8f,b.height+4f)}
 ...
 fun draw(canvas:Canvas,node:MathNode,x:Float,top:Float,size:Float,color:Int=android.graphics.Color.BLACK):Float{when(node){
  ...
  is MathNode.Radical->{val b=measure(node.body,size);val p=paint(size,false,color);canvas.drawText("√",x,top+size,p);val bx=x+size*.7f;canvas.drawLine(bx,top+2,bx+b.width,top+2,p);draw(canvas,node.body,bx,top,size,color);return b.width+size*.8f}
