...
private fun MathNodeView(node:MathNode,size:TextUnit,color:Color){when(node){
    ...
    is MathNode.Radical->Row(verticalAlignment=Alignment.CenterVertically){Text("√",fontSize=size*1.25f,color=color);Box(Modifier.border(width=.7.dp,color=color).padding(2.dp)){MathNodeView(node.body,size,color)}}
