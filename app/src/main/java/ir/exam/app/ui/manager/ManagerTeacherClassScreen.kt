...
fun ManagerTeacherClassScreen(teacherId:String,onBack:()->Unit){
 ...
 var createOpen by remember{mutableStateOf(false)};var name by remember{mutableStateOf("")};var grade by remember{mutableStateOf("")};var field by remember{mutableStateOf("")}
 ...
 Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
  ...
  if(selected==null){
   Button(onClick={createOpen=!createOpen},modifier=Modifier.fillMaxWidth()){Icon(Icons.Outlined.Add,null);Text("کلاس جدید برای معلم")}
   if(createOpen){Card(Modifier.fillMaxWidth()){Column(Modifier.padding(12.dp),verticalArrangement=Arrangement.spacedBy(7.dp)){OutlinedTextField(name,{name=it.take(120)},label={Text("نام کلاس")});OutlinedTextField(grade,{grade=it.take(120)},label={Text("پایه")});OutlinedTextField(field,{field=it.take(120)},label={Text("رشته")});Button(enabled=name.isNotBlank(),onClick={scope.launch{repo.createTeacherClass(teacherId,name,grade,field).onSuccess{name="";grade="";field="";createOpen=false;loadClasses()}.onFailure{error=safeManagerError(it)}}}){Text("ایجاد")}}}}
   classes.forEach{c->Card(Modifier.fillMaxWidth()){Row(Modifier.fillMaxWidth().padding(12.dp),verticalAlignment=Alignment.CenterVertically){Column(Modifier.weight(1f)){Text(c.name,style=MaterialTheme.typography.titleMedium);Text("${c.grade} ${c.field} · ${c.total} دانش‌آموز")};Button(onClick={loadRoster(c)}){Text("ورود")};IconButton(onClick={scope.launch{repo.deleteTeacherClass(c.id).onSuccess{loadClasses()}.onFailure{error=safeManagerError(it)}}}){Icon(Icons.Outlined.Delete,"حذف کلاس",tint=MaterialTheme.colorScheme.error)}}}}
  }else{
