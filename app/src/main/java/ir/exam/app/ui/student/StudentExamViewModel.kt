...
class StudentExamViewModel(private val exams:ExamRepository, private val drafts:AnswerDraftRepository):ViewModel() {
    ...
    private fun startTimer(){ timer?.cancel(); timer=viewModelScope.launch { while(state.value.remainingSeconds>0 && !state.value.finished){delay(1000);_state.update{it.copy(remainingSeconds=(it.remainingSeconds-1).coerceAtLeast(0))}}; if(state.value.remainingSeconds==0) submit() } }
