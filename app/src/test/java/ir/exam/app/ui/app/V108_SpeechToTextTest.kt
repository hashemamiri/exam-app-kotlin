...
/** V108–V112 — میکروفون گفتار به متن کنار آیکن تصویر (پنجرهٔ سیستمی، فقط متن). */
class V108_SpeechToTextTest {
    ...
    fun `mic button is wired next to the image icon and only inserts plain text`() {
        ...
        assertTrue("فقط تبدیل متنی استفاده نمی‌شود", "SpeechMathConverter.convertPlain(best)" in button)
        assertFalse("مبدل فرمول در دکمه استفاده می‌شود", "SpeechMathConverter.convert(" in button || "toLatexOnly" in button)
        assertTrue("زبان فارسی نیست", "launchRecognizer(\"fa-IR\")" in button)
        assertTrue("زبان انگلیسی نیست", "launchRecognizer(\"en-US\")" in button)
    }

    @Test
    fun `uses the system speech dialog - no in-app SpeechRecognizer loop`() {
        val button = source("app/src/main/java/ir/exam/app/ui/speech/SpeechToTextButton.kt")
        assertTrue("RecognizerIntent.ACTION_RECOGNIZE_SPEECH" in button)
        assertTrue("ActivityResultContracts.StartActivityForResult()" in button)
        assertFalse("حلقهٔ SpeechRecognizer داخل برنامه هنوز هست", "SpeechRecognizer.createSpeechRecognizer" in button || "RecognitionListener" in button)
        assertFalse("LinearProgressIndicator" in button || "onPartialResults" in button || "onRmsChanged" in button)
        assertTrue("ویرایشگر با لمس بیرون بسته می‌شود", "onDismissRequest = {}," in button)
