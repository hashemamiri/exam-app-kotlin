...
**آخرین به‌روزرسانی:** ۲۰۲۶-۰۹-۰۸ — V115 هدر واقعاً ثابت + پیش‌نمایش چندبرگی A4
**زبان همکاری:** فارسی
...
## ۱۱) قانون دائمی تحویل نسخه‌ها: اسکریپت خودکار و مستقل پایتون (`apply_vXX.py`)

از نسخهٔ V98 به بعد، به دستور صریح کاربر و برای جلوگیری از خطاهای متداول `git apply` (نظیر تفاوت کامیت مبدأ، تعارض‌های متنی، تفاوت سطرها و وجود فایل‌های untracked محلی)، **روش استاندارد و قطعی تحویل تمام نسخه‌ها استفاده از اسکریپت خودکار، مستقل و خود-استخراج‌گر پایتون (`apply_vXX.py`) است.**

### ساختار و نحوهٔ کار اسکریپت `apply_vXX.py`:
1. **خود-استخراج‌گر و مستقل (Self-Contained):** فایل‌های تغییریافته و جدید به‌صورت آرشیو فشردهٔ Gzip-Tar در قالب رشتهٔ Base64 درون اسکریپت قرار می‌گیرند و بدون نیاز به اینترنت یا ابزار خارجی مستقیماً در مسیر پروژه استخراج می‌شوند.
2. **مصونیت ۱۰۰٪ از تعارض‌های Git:** مستقل از اینکه شاخهٔ محلی کاربر روی کدام کامیت قبلی باشد، بدون خطای patch conflict اجرا می‌شود.
3. **راستی‌آزمایی خودکار:** بلافاصله اسکریپت `scripts/verify_native_final.py` را اجرا کرده و از سلامت کامل کد اطمینان حاصل می‌کند.
4. **بررسی پاکیزگی و نبود خطای نحوی:** دستور `git diff --check` را اجرا می‌کند.
5. **تمیز نگه‌داشتن درخت کاری پروژه:** اسکریپت نصاب پس از اعمال، فایل موقت نصاب را از پوشهٔ پروژه پاک می‌کند تا هیچ فایل اضافی در مخزن کامیت نشود.
6. **کامیت و انتشار خودکار:** تغییرات را با پیام فارسی استاندارد کامیت کرده و با `git push origin HEAD` به مخزن گیت‌هاب ارسال می‌کند.

### فرمت استاندارد دستور WSL برای اجرای هر نسخه:

```bash
cd /mnt/c/Users/Hashem/Downloads/exam-app-kotlin && python3 /mnt/c/Users/Hashem/Downloads/apply_vXX.py
```

### الزامات هر تحویل:
- ارائه فایل `apply_vXX.py` کامل و باز کردن آن با `present_file` برای دانلود مستقیم.
- ارائه فایل SQL در صورت نیاز به مهاجرت پایگاه داده.
- توضیح فارسی کوتاه و شفاف از تغییرات.
- به‌روزرسانی اجباری `docs/fa/HANDOFF_KOTLIN_MIGRATION_FA.md` در همان نسخه.
- اجرای موفق `verify_native_final.py` و تست‌های واحد `testDebugUnitTest`.
...
## ۲۶۶) V70.2 — رفع «قالب نامعتبر است» هنگام بازکردن PDF مستقیم پرینتر

### گزارش کاربر

```text
پس از ذخیره با آیکن پرینتر و باز کردن فایل pdf، خطای «قالب نامعتبر است» می‌دهد.
پاسخ ask_user: پیام «فایل PDF ساخته شد» نشان داده می‌شود ولی فایل ذخیره‌شده ۰ کیلوبایت/خیلی کوچک است.
```

### ریشه (اثبات‌شده، نه حدس)

```text
- رشتهٔ «قالب نامعتبر است» در سورس اپ نیست → پیام viewer سیستم است یعنی خود فایل خراب/خالی است.
- شبیه‌سازی JVM (openPDF 1.3.43 + همین فونت‌های bnazanin + همین کد) در مسیر قدیمی و مسیر جدید،
  PDF معتبر می‌سازد (repro.pdf حدود ۲۸KB؛ pypdf آن را می‌خواند) → کتابخانه/فونت سالم‌اند.
- علت: DirectPdfExporter خروجی را «مستقیم» روی استریم SAF می‌نوشت
  (stream.use { buildPdf(withImages, it) }). هر خطای میانی یا provider که نوشتنِ
  تدریجی را کامل ثبت نکند، فایل ۰ بایتی/ناقص در محل انتخابی باقی می‌گذاشت؛ چون
  runCatching فقط خطای «خودِ فراخوانی» را می‌گرفت نه «۰ بایتی ماندن فایل»، پیام
  موفقیت هم نشان داده می‌شد — دقیقاً همان چیزی که کاربر گزارش کرد.
- بقیهٔ خروجی‌های اپ (XLSX/JSON/CSV/backup) با الگوی «ساخت کامل در حافظه ← یک‌جا
  نوشتن بایت‌ها» کار می‌کنند و این مشکل را ندارند.
```

### چه شد

```text
- export() بازنویسی شد: PDF کامل ابتدا در ByteArrayOutputStream ساخته می‌شود
  (buildPdf(withImages, buffer) سپس buffer.toByteArray()) و فقط پس از موفقیت ساخت،
  بایت‌های کامل یک‌جا با it.write(bytes) + it.flush() روی محل انتخابی SAF نوشته می‌شوند.
- در صورت شکست ساخت، فایل ۰ بایتیِ ایجادشده با contentResolver.delete(target, null, null)
  حذف می‌شود تا بازکردنش خطای «قالب نامعتبر» ندهد.
- قالب، فونت‌ها و همهٔ قابلیت‌های چاپ (چاپ برگه، چاپ با کلید، ویرایش سند) دست‌نخورده‌اند.
- حافظهٔ موقت معادل حجم PDF (چند صد کیلوبایت تا چند مگابایت بسته به تصویرها) است —
  همان الگوی بقیهٔ خروجی‌های برنامه.
```
...
```text
تغییر:
app/src/main/java/ir/exam/app/core/printing/DirectPdfExporter.kt   (export اتمیک: حافظه → نوشتن یک‌جا → حذف در شکست)
app/src/test/java/ir/exam/app/ui/app/V70_2DirectPdfAtomicWriteTest.kt  (افزوده)
scripts/verify_native_final.py                                     (باند V70.2 + startswith چنج‌لاگ → V70.2:)
text/CHANGELOG_FA.txt                                              (خط V70.2 در بالا؛ خط V70.0 حفظ شد)
docs/fa/HANDOFF_KOTLIN_MIGRATION_FA.md                             (همین بخش ۲۶۶)
```
...
```text
شبیه‌سازی JVM مسیر جدید (BAOS → write bytes → pypdf): PASS (۱ صفحه، متن قابل استخراج)
FINAL_NATIVE_VERIFY → PASS (باند V70.2 + همهٔ باندهای قبلی)
```

### چک‌لیست دستگاه

```text
۱) چاپ آزمون → آیکن پرینتر → انتخاب محل ذخیره؛ فایل PDF باید حجم غیرصفر (چند ده KB به بالا) داشته باشد.
۲) بازکردن همان فایل — نباید «قالب نامعتبر است» بدهد.
...
## ۲۷۴) V72.0.5 — رفع آینه‌شدن ترتیب شکل‌های inline در ویرایشگر

### گزارش دستگاه

```text
در تصویر ویرایشگرِ بدون انتخاب، ترتیب افقی شکل‌ها نسبت به چاپ برعکس بود:
جدول به سمت چپ می‌رفت و شکل/آناتومی به سمت راست؛ اما پس از انتخاب سؤال،
همان محتوا با ترتیب دیگری دیده می‌شد.
```

### ریشه

```text
برای شکل‌های درون‌خطی، EngineObjectsLayer از
layout.getPrimaryHorizontal(mark.charOffset) به‌عنوان ضلع چپ مستطیل استفاده
می‌کرد. در پاراگراف RTL این مختصات همیشه ضلع چپ نیست و می‌تواند مرز راست
ReplacementSpan باشد؛ نتیجه، آینه‌شدن ترتیب جدول/آناتومی/شکل در لایهٔ Compose
بود، در حالی که چاپ رسمی از خود StaticLayout رسم می‌کرد.
```

### اصلاح

```text
- تابع replacementLeftPt هر دو مرز span یعنی offset و offset+1 را می‌خواند و
  کمینهٔ آن‌ها را به‌عنوان ضلع چپ واقعی می‌گیرد.
- مختصات به محدودهٔ همان خط StaticLayout محدود می‌شود تا شکل از عرض محتوا خارج
  نشود.
- editorObjects و hitTest دقیقاً از همین تابع مشترک استفاده می‌کنند؛ بنابراین
  جای لمس و جای نمایش شکل یکی است.
- مسیر fallback Compose حفظ شد تا شکل‌ها روی دستگاه‌هایی که ReplacementSpan را
  در Canvas پیش‌نمایش نمی‌کنند گم نشوند.
- دکمه‌های آبی چاپ برگه/چاپ با کلید به Android Print Framework و
  OfficialPdfPrintAdapter باقی ماندند؛ هیچ انتقالی به iText 7 انجام نشد.
- مسیر مستقل آیکن PDF با iText 7 نیز دست‌نخورده ماند.
```
...
```text
app/src/main/java/ir/exam/app/core/printing/OfficialPdfPrintAdapter.kt
app/src/test/java/ir/exam/app/ui/app/V68_9UnifiedEngineWysiwygTest.kt
scripts/verify_native_final.py
text/CHANGELOG_FA.txt
docs/fa/HANDOFF_KOTLIN_MIGRATION_FA.md
```
...
```text
۱) ویرایشگر را باز کنید و بدون لمس سؤال، ترتیب جدول/آناتومی/شکل را با چاپ
   رسمی مقایسه کنید.
۲) همان سؤال را لمس کنید؛ محتوای سؤال نباید ناپدید یا از نظر ترتیب آینه شود.
...
## ۲۸۱) ممیزی امنیتی جامع بعد از V74.2 — ردفلگ‌ها، باگ‌های امنیتی و نقشهٔ سخت‌سازی

### چرا این بخش اضافه شد

```text
درخواست کاربر: «سورس رو بررسی کن و ردفلگ یا هر چیز دیگه‌ای رو پیدا کن؛ باگ‌های
امنیتی رو هم پیدا کن؛ هر چیزی که امنیت برنامه رو به خطر میندازه یا برنامه رو در
معرض هک قرار میده رو لیست کن.»
روش: بازخوانی مستقیم سورسِ HEAD 2e7f0e7 (۳۴۸ فایل Kotlin، ۶۸ فایل SQL، ۳ Edge
Function، workflow، manifest و assetها). نه حدس، نه اجرای کد روی سرور واقعی.
```

### وضعیت دقیق مخزن در لحظهٔ ممیزی

```text
HEAD:                 2e7f0e7  (V74.2 — حذف کامل iText 7)
برنچ:                 main
آخرین اجرای CI:       run 415 → success
فایل‌های تحت نسخه:     ۶۵۴  |  Kotlin: ۳۴۸  |  SQL: ۶۸
اسکنر پروژه:          FINAL_NATIVE_VERIFY=PASS  kotlin_files=198  edge_functions=3
منبع حقیقت:           همین فایل هندآف داخل ریپو (نسخهٔ جداگانه‌ای معتبر نیست)
```
...
### قانون دائمی پس از این بخش

```text
۱) هر پچ امنیتی باید ابتدا ردیف مربوط به خود را در جدول بالا به «انجام‌شده»
   تغییر دهد و شمارهٔ پچ را جلوی آن بنویسد.
۲) تا زمانی که بند ۱.۱ (sandbox پرداخت) روی سرور واقعی بسته نشده، هیچ پچ
   امنیتی دیگری «پایانِ کار امنیت» تلقی نمی‌شود.
...
## ۲۸۴) V75.2 — محدودسازی افشای ایمیل کادر مدرسه (بند ۳.۱ گزارش امنیتی)

### ریشه

```text
native_staff_login_email_v1 به anon مجاز است (grant ... to anon, authenticated) و از
auth.users ایمیل کامل معلم/مدیر را برمی‌گرداند. برای «ورود با نام کاربری» این تابع
لازم است: کلاینت ابتدا نام کاربری را به ایمیل تبدیل می‌کند و سپس با رمز وارد می‌شود.
بدون محدودیت نرخ، مهاجم می‌توانست فهرست نام‌های کاربری را جست‌وجو کند و ایمیل ورود
را برای حملهٔ رمز/فیشینگ برداشت کند (Username Enumeration).
```

### تصمیم

```text
حذف grant یا حذف تابع مسیر ورود با نام کاربری را می‌شکند؛ بنابراین تابع حفظ شد و به‌جای
آن «محدودسازی نرخ» اضافه شد (الگوی استاندارد برای endpointهای حساس به شمارش).
```

### چه شد

```text
۱) جدول public.native_staff_login_attempts با RLS فعال (بدون policy ⇒ فقط توابع
   security definer به آن دسترسی دارند).
۲) پیش از هر پاسخ: پاک‌سازی ردیف‌های قدیمی‌تر از ۲ ساعت، سپس شمارش تلاش‌ها.
۳) آستانه‌ها: بیش از ۵ تلاش برای یک نام کاربری در ۱۰ دقیقه، یا بیش از ۲۰ تلاش
   کلی در یک دقیقه ⇒ خطای عمومی «تلاش‌های ورود بیش از حد است».
۴) چون تابع اکنون INSERT دارد، نشانگر stable حذف شد (در غیر این صورت PostgreSQL
   برای تابعِ stable اجازهٔ نوشتن نمی‌دهد).
۵) پیام خطا در حالت نامعتبر همان پیام قبلی است (عدم افشای وجود نام کاربری).
```

### فایل‌ها

```text
supabase/migrations/20260903_native_staff_login_throttle_v75_2.sql   (جدید)
sql/manual/SQL_NATIVE_STAFF_LOGIN_THROTTLE_V75_2.sql                 (جدید، dual-write)
app/src/test/java/ir/exam/app/ui/app/V75_2StaffLoginThrottleTest.kt   (جدید)
scripts/verify_native_final.py
text/CHANGELOG_FA.txt
docs/fa/HANDOFF_KOTLIN_MIGRATION_FA.md
```
...
ریسک باقیمانده (ثبت‌شده): مهاجمِ صبور با سرعتِ زیر آستانه هنوز می‌تواند نام‌های
کاربری را حدس بزند؛ بستن کامل نیازمند حذف ورود با نام کاربری یا CAPTCHA است.
```

## ۲۸۵) V75.3 — محافظ ورودیِ مانیتورینگ آزمون (بند ۳.۲ گزارش امنیتی)

### ریشه

```text
native_monitor_upsert_v1 فقط auth.uid() را چک می‌کرد:
  الف) وجود آزمون بررسی نمی‌شد ⇒ هر کاربر می‌توانست برای هر exam_id دلخواه ردیف
      بسازد و پایگاه را با کلیدهای ساختگی پر کند؛
  ب) p_report هیچ محدودیت اندازه‌ای نداشت و در برخورد با ردیف موجود با عملگر ||
      روی گزارش قبلی جمع می‌شد ⇒ رشد نامحدود و امکان درج دادهٔ جعلی در گزارشی
      که معلم در صفحهٔ مانیتور می‌بیند.
```

### چه شد

```text
۱) وجود آزمون با select 1 from public.exams where id = p_exam الزامی شد.
۲) سقف هر گزارش: ۸۱۹۲ بایت (octet_length روی متن jsonb).
۳) سقف تعداد آیتم‌های هر گزارش: ۱۰۰ کلید (jsonb_object_keys).
۴) سقف مجموعِ ذخیره‌شده برای هر دانش‌آموز در هر آزمون: ۳۲۷۶۸ بایت.
۵) رفتار عادی (درج/به‌روزرسانی/ادغام) برای گزارش‌های معمولی تغییر نکرده است.
```

### فایل‌ها

```text
supabase/migrations/20260903_native_exam_monitor_limits_v75_3.sql   (جدید)
sql/manual/SQL_NATIVE_EXAM_MONITOR_LIMITS_V75_3.sql                 (جدید، dual-write)
app/src/test/java/ir/exam/app/ui/app/V75_3ExamMonitorLimitsTest.kt   (جدید)
scripts/verify_native_final.py
text/CHANGELOG_FA.txt
docs/fa/HANDOFF_KOTLIN_MIGRATION_FA.md
```
...
## ۲۹۱) V75.9 — اصلاحیهٔ گزارش امنیتی و جدول پیگیری

### اصلاحیه: موردِ «ذخیرهٔ رمز دانش‌آموز»

```text
عنوانِ این مورد در گزارش اولیه گمراه‌کننده بود. بررسیِ دقیق‌ترِ کد نشان داد:
  - StudentPasswordVault رمزها را با کلیدِ غیرقابل‌استخراجِ Android Keystore و
    الگوریتم AES/GCM رمزنگاری می‌کند؛ در فایل تنظیمات فقط IV و ciphertext است.
  - allowBackup=false است، بنابراین این داده به دستگاهِ دیگر منتقل نمی‌شود.
آنچه واقعاً هست «قابلیتِ بازیابی روی همان دستگاه» است (یک تصمیمِ طراحی برای راحتیِ معلم)،
نه ذخیرهٔ متن ساده. با تصمیمِ شما این رفتار حفظ می‌شود.
```

### ریسکِ باقیمانده (ثبت‌شده، پذیرفته‌شده)

```text
اگر دستگاهِ معلم روت‌شده یا به بدافزار آلوده باشد و هم‌زمان قفلِ صفحه دور زده شود،
مهاجم می‌تواند رمزها را بازیابی کند. راهکارِ کامل: نمایشِ یک‌باره و نگه‌داری در حافظه،
که به معنای واردکردنِ دوبارهٔ رمزِ دانش‌آموزان پس از هر بار بستنِ کاملِ برنامه است.
هر زمان خواستید، این تغییر یک پچِ کوچک است.
```

### جدول پیگیری

```text
در انتهای docs/fa/SECURITY_AUDIT_V74_2_FA.md یک «جدول پیگیری اصلاحات» اضافه شد که
برای هر مورد: نسخهٔ اصلاح‌کننده، وضعیت و عملیاتِ دستیِ لازم را نشان می‌دهد.
مرجعِ معتبر همان فایل است (نسخهٔ درون‌یافت‌شده در بخش ۲۸۱ قدیمی‌تر است).
```

### فایل‌ها

```text
docs/fa/SECURITY_AUDIT_V74_2_FA.md
text/CHANGELOG_FA.txt
docs/fa/HANDOFF_KOTLIN_MIGRATION_FA.md
```
...
## ۲۹۵) V76.1 — رفع چهار ایراد آزمون‌ساز از بازخورد دستگاه + بازگشت ۷ ابزار

### گزارش کاربر (عین متن)

```text
دکمه فرمول کار نمی‌کنه و دکمه عکس زیر دکمه فرمول کار نمی‌کنه. آیکن چشم
پیش‌نمایش کار نمی‌کنه و پنجره دکمه سوال جدید کوچک است. ۷ آیکن حذف‌شده را
از داخل بخش ایجاد آزمون اضافه کن.
```

### ریشه‌یابی (بدون حدس — از کد asset)

```text
۱) فرمول: const EXACT_MATH_EDITOR_B64 = '' خالی است — ویرایشگر iframe در
   خروجی تک‌فایلیِ سایت اصلاً embed نشده؛ bootExactMathEditor بعد از ۹
   ثانیه alert «بارگیری نشد» می‌دهد. در هیچ نسخه‌ای از فایل (حتی اصلی)
   کار نمی‌کرد.
۲) عکس: دکمهٔ 📷 (qimg-cam «افزودن تصویر به سؤال») روی input[type=file]
   داینامیک click() می‌زند؛ WebView اندروید بدون onShowFileChooser آن را
   بی‌صدا نادیده می‌گیرد.
۳/۴) چشم و کوچکی پنجره‌ها: کدِ چشم (togglePreviewWindow) سالم است؛ مشکل
   رندر بود — ExamHtmlPrintDialog بدون useWideViewPort/loadWithOverview
   متاوویوپورتِ فایل (width=device-width) را اعمال نمی‌کند ⇒ صفحه با
   مقیاس دسکتاپی کوچک می‌شود؛ تارگت‌های لمس ریز و پنجره‌ها (qtype-box
   min(620px,94vw)) نیم‌مقیاس دیده می‌شوند.
۵) ۷ ابزار: همهٔ ماژول‌ها (gf/gr/tb/an/pt/sc + GeoFig) زنده و bind هستند
   (بررسی: bindToolbar، \$('openGraphEditor')…، overlayهای داینامیک) —
   حذفِ دکمه‌ها در V76.0 قابل برگشت بود.
تأیید کلید کاربر: راست‌آزمایی «فرمول با \(…\) رندر می‌شود»:
renderPlainWithMath ریگکس \\(…\\) را با mathToHtml (MathParser کامل)
رندر می‌کند ⇒ خروجی مودالِ ساده در پیش‌نمایش و چاپ کامل است.
```

### اصلاح‌ها

```text
- asset بازتولید شد (P2، ۶٬۱۷۳٬۲۷۴ chars؛ /tmp/transform_asset.py): گام‌های
  حذف دکمه‌ها/CSS/خنثی‌سازی ویرایشگر شکل حذف شد (هر ۷ ابزار + ویرایشگر
  شکل کامل برگشت)؛ گام جدید «fallback فرمول»: قبل از boot، اگر B64 خالی
  بود window.__openFallbackMathModal(t, capturedTex) مودال سادهٔ خود فایل
  (mathModal: نمادها + پیش‌نمایش زنده + درج) را باز می‌کند؛ توکنِ
  ویرایش مجدد (t._qmfPending) هم بازه را انتخاب می‌کند تا درج، جایگزین
  فرمول قبلی شود.
- ExamHtmlPrintDialog: settings.useWideViewPort/loadWithOverviewMode +
  onShowFileChooser با rememberLauncherForActivityResult(GetContent)
  (accept image/*) → دکمهٔ دوربین کار می‌کند.
- تست‌ها: V76_0 تست asset بازنویسی (۷ ابزار + fallback + ویرایشگر شکل
  دست‌نخورده) + تست جدید viewport/انتخاب فایل؛ V73_0 سوزن‌های حذف آیکن
  معکوس شدند؛ verify: V73.0/V76.0 بلوک‌های asset معکوس+fallback و
  require جدید دیالوگ.
- node: fallback واقعی اجرا شد (prefill tex، بازهٔ 3..9 برای جایگزینی،
  focus، alert بدون فیلد) = PASS؛ node --check همهٔ بلوک‌های JS تمیز
  (تنها false-positive: بلوک b64 استودیو که JS نیست).
```

### درس‌ها

```text
- هر ادعای «کار نمی‌کند» سه لایه دارد: کدِ فایل، بافتِ WebView، مقیاس
  رندر — این‌بار هر سه لایه ریشه‌یابی شد؛ کوچکیِ ظاهری می‌تواند دروغگاه
  «دکمه کار نمی‌کند» باشد (تارگت ریز = لمس ناموفق).
- خروجی «ذخیره فایل» سایت‌ها ممکن است ثابت‌های embed را خالی بگذارد
  (EXACT_MATH_EDITOR_B64) — پیش از اتکا به یک مسیر، محتوای ثابت را چک کن.
- input[type=file] داینامیک در WebView اندروید همیشه به onShowFileChooser
  نیاز دارد؛ بدون آن بی‌صدا می‌میرد (بدون هیچ خطایی).
- معکوس‌کردن سوزن‌های «حذف‌محور» هنگام برگشت قابلیت فراموش نشود (V73.0
  و V76.0 هر دو block ویرایش شدند + پیام require به‌روز).
```

پچ: V76_1_builder30_tools_and_fixes — بدون SQL.

## ۲۹۶) V76.2 — فرمولِ دقیقِ نسخهٔ ۳۰ + ریشهٔ واقعی ریزبودیِ پنجره‌ها

### گزارش کاربر

```text
۷ آیکن اضافه‌شده کار نمی‌کنند. آیکن فرمول دقیقاً مطابق نسخه ۳۰ باشد.
آیکن چشم پیش‌نمایش هنوز کار نمی‌کند. پنجره سوال جدید هنوز کوچک است.
```

### ریشه‌یابی (اجرای واقعی asset در jsdom — نه حدس)

```text
- jsdom روی assetِ V76.1: چشم باز/بسته می‌شود، هر ۷ ابزار overlay باز
  می‌کنند، بدون خطای JS ⇒ کدِ فایل سالم است؛ مشکل سمتِ بافتِ WebView بود.
- فرمول: کشفِ مهم — ماژول QMF خودش ویرایشگرِ اصلیِ نسخهٔ ۳۰ را دارد:
  boot() → doc.write(MATH_EDITOR_HTML) (۱۱۲KB، ویرایشگر مستقل 🧮) با
  pollِ openMath/mfApply/closeMath. گاردِ «!EXACT_MATH_EDITOR_B64» در
  P2 این مسیرِ سالم را می‌دزدید و به مودالِ ساده می‌فرستاد ⇒ کاربرِ درست
  می‌گفت «دقیقاً مطابق نسخه ۳۰ باشد». سیستمِ B64/ExamEditor
  (bootExactMathEditor) کدِ مرده است (فقط تعریف، بدون caller).
- ریزبودیِ پنجره‌ها: loadWithOverviewMode=true در WebView با محتوای عریضِ
  A4 (۱۹۴mm ≈ ۷۳۳px) کل صفحه را zoom-out می‌کند ⇒ همه‌چیز ریز، تپ‌ها
  نامطمئن، پنجره‌ها کوچک. jsdom چون layout ندارد این را نشان نمی‌داد.
```

### اصلاح‌ها

```text
- asset (P3، ۶٬۱۷۳٬۹۲۹ chars): گاردِ P2 حذف شد ⇒ fx از مسیرِ واقعی
  MATH_EDITOR_HTML می‌رود (jsdom: بوت OK، qTxt_1 پر، فریم نمایان).
  پشتیبانِ ساده فقط در timeoutِ شکستِ بوت (۶ ثانیه) به‌عنوان آخرین‌چاره.
- mobile CSS (qmfMobileWindows): در عرض ≤۷۰۰px پنجرهٔ «سوال جدید» ۹۸vw با
  آیتم‌های ۲ ستونهٔ بزرگ، پنجرهٔ چشم ۹۸vw/۹۴vh با zoom:.44 برای هم‌عرض شدن
  A4، math-box تمام‌عرض.
- Kotlin: settings.loadWithOverviewMode = false (useWideViewPort بماند).
- jsdom نهایی (drive2.js): ۱۵/۱۵ OK — فرمول اصلی ۴ ادعا، ۷ ابزار، چشم،
  پنجرهٔ بزرگ، CSS موبایل (تنها خطاها: canvas/focusِ خودِ jsdom).
```

### درس‌ها

```text
- jsdom برای «کد درست است» عالی است ولی برای «روی دستگاه درست دیده
  می‌شود» نه — layout/zoom ندارد؛ ادعای viewport را باید با
  loadWithOverviewMode=false + CSS موبایلِ درون‌فایلی هم پوشش داد.
- قبل از افزودن fallback، ببین مسیرِ اصلی «از قبل» چه منبعی دارد
  (MATH_EDITOR_HTML) وگرنه مسیرِ سالم را می‌دزدی (بالاتر: P2).
- سیستم‌های مرده (bootExactMathEditor با B64 خالی) را با شمارشِ caller
  شناسایی کن؛ برای ادعای «خرابی» فقط مسیرِ زنده مهم است.
- اگر بعد از این پچ هم روی دستگاه چیزی کار نکرد: اسکرین‌شات از همان
  لحظه + نسخهٔ WebView دستگاه (chrome://version در تنظیمات) لازم است.
```
...
## ۳۰۰) V76.5 — ابزارهای صفحه/صاف‌سازی بومی (برش ۴گوشه + ±۱۵° + خودکار)

### چه شد (ادامهٔ نقشهٔ ۲۹۸)

```text
- 📐 صفحه‌ای (۴ گوشه): چیتِ حاضر در نوار ابزار → پیش‌نمایش تصویرِ خام با
  چهار نقطهٔ شماره‌دار ۱..۴ (کشیدنی؛ آستانهٔ لمس ۲۲۰px) + چهارگوشِ زرد؛
  «✓ اعمال صاف‌سازی» با Matrix.setPolyToPoly (۴ نقطه = پرسپکتیو کامل)
  به مستطیلِ محصورکننده وارپ می‌کند و به original می‌نشیند؛ چرخش/قرینه/
  صاف‌سازی/برش ریست و از صفحهٔ صاف ادامه می‌دهد.
- ↯ صاف‌سازیِ دقیق: اسلایدر −۱۵..+۱۵ (گام ۰٫۱) + «صفر کردن» + شبکهٔ
  راهنمای افقی (deskewGrid) روی پیش‌نمایش؛ در processAndEncode داخل
  همان ماتریسِ چرخش اعمال می‌شود (postRotate(rotation + deskew)).
- 🎯 تشخیص خودکار زاویه: نمونه‌برداری ~۲۰۰px → سیاه‌سفید با آستانهٔ
  جاری → برای هر زاویهٔ ۰٫۵±۱۵ شیبِ ردیف‌ها (y + x·tanθ) → واریانسِ
  تاریکیِ ردیف‌ها؛ بیشینه = زاویه؛ نتیجه در اسلایدر می‌نشیند و پیام
  می‌دهد (در Dispatchers.Default، با processing).
- پیش‌نمایش deskew را زنده می‌بیند؛ pointerInput حالا با
  (aspect, perspMode, boxSize) کلید می‌خورد تا هندسهٔ کهنه نماند.
```

### نکات فنی

```text
- DrawScope.drawIntoCanvas + nativeCanvas.drawText برای شمارهٔ نقاط
  (بدون TextMeasurer).
- Kotlin: xs.min() قدیمی است — minOrNull یا return@runCatching null.
- وارپ موفق ⇒ حالت‌ها ریست؛ وارپ نامعتبر ⇒ note خطا و بدون تغییر.
```

### مسیر باقی‌ماندهٔ بومی‌سازی (تعهد ۲۹۸)

```text
V76.6: تفکیک چندسؤاله + مدیریت چند تصویر (فهرست/حذف/اندازهٔ هر تصویر).
V76.7: پاک‌کن/لاک‌گیر + برچسب متنی و فلش + مقایسهٔ قبل/بعد.
V76.8: حذف استودیوی HTML (qimgStudioSrc) و «ابزارهای کامل» موقت.
```
...
## ۳۰۵) V76.7 — ابزارهای رسم بومی (لاک‌گیر/برچسب/فلش) + قبل/بعد

### پوشش (مطابق مجموعهٔ استودیو)

```text
➡️ فلش / ↔️ فلش دوسر / 📏 خط / ⬜ کادر / ⭕ بیضی / ✏️ خط آزاد /
🖍️ هایلایتر نیمه‌شفاف / 🚫 سانسور / 🔤 برچسب متنی (دیالوگ ورودی متن)
۴ رنگ: 🔴 🔵 ⚫ 🟢 — 👆 انتخاب/جابجایی — ↩️ بازگردانی / ↪️ انجام مجدد /
🗑️ حذف انتخاب / 🧹 پاک کردن همه — 👁 قبل/بعد (نمای تصویرِ خامِ ورودی).
در پچ‌های بعدی از خانواده: قطره‌چکان رنگ، لایهٔ پیشرفتهٔ اشیاء
(گروه/ترتیب/تکثیر)، فلش‌های منحنی.
```

### معماری

```text
- StudioShape(type, points نرمال‌شده, color:Int, text) — مختصات ۰..۱
  نسبت به تصویرِ نمایش‌داده‌شده تا پیش‌نمایش و پخت یکسان بمانند.
- تعامل: drawMode != none → کشیدن شکل زنده (free/highlighter نقطه‌جمع
  می‌کنند)؛ رویCommit در onDragEnd فقط شکلِ معنادار (طول/نقاط کافی)
  به لیست می‌رود. drawMode = none → hit-test جعبهٔ نقاط با حاشیهٔ ۳٫۵٪
  → انتخاب (حلقهٔ نشانگر) و جابه‌جایی با delta نرمال‌شده.
- متن: لمس در حالت 🔤 → دیالوگ AlertDialog+OutlinedTextField → شکلِ
  text در نقطهٔ لمس (رسم با nativeCanvas در پیش‌نمایش، Paint در پخت).
- bakeShapes(base, shapes): android.graphics.Canvas — هایلایتر
  alpha=110، سانسور = پیکسلیِ واقعی (میانگین بلوک ~min/8)، فلش‌ها با
  atan2 و سرِ ۳٫۲×ضخامت، متن با textSize=4.5٪ ارتفاع.
- زنجیره: encodeCropped پارامتر shapes دارد؛ پخت قبل از برش ⇒ درجِ
  تکی، جایگزینی و هر بخشِ تفکیک همه شکل‌ها را دارند. processAndEncode
  هم shapes را از دکمهٔ تایید می‌گیرد.
- قبل/بعد: openedWith در هر سه نقطهٔ ورود (دوربین/گالری/ویرایشِ موجود)
  ثبت می‌شود؛ چیپ 👁 پیش‌نمایش را به تصویر خام سوییچ می‌کند و پوشش‌ها
  را هم مخفی می‌کند (شاخهٔ previewOriginal بالای زنجیرهٔ Canvas).
- ریستِ کامل stateهای رسم در هر بارگذاری تصویر جدید (وگرنه شکل‌های
  سؤال قبلی به تصویر بعدی می‌چسبیدند).
```

### درس‌ها

```text
- پین‌های pointerInput در V76.5/V76.6 با هر توسعهٔ کلید کهنه شدند —
  قانونِ «همهٔ تست‌های خواننده پس از آخرین ویرایش» دوباره اجرا شد
  (شبیه‌ساز هر دو را گرفت: ۳۰۱ سوزن سبز).
- تست V76_7 این‌بار از متن واقعی فایل تولید شد (۴۳ پین استخراج و
  راستی‌آزمایی‌شده) — الگوی درست برای آینده: اول کد، بعد پین از کد.
```
...
## ۳۰۸) V76.8 — پاک‌سازی کدِ مردهٔ ویرایشگر فرمول دقیق در asset

CI برای V76.7.2 **کاملاً سبز** شد (کامپایل + ۷۲۳ تست + lint + انتشار). چون قدم بعدیِ
بزرگ (حذف استودیوی HTML) نیازمند تصمیم اسکوپِ کاربر است، این پچ همان کارِ
کم‌ریسکِ معوقِ فهرست نگهداری را انجام می‌دهد.

مرزبندیِ دقیق پیش از حذف (بدون حدس):
- `const EXACT_MATH_EDITOR_B64 = '';` → رشتهٔ **خالی** بود.
- `exactMathEditorHtml()` تنها مصرف‌کنندهٔ آن بود و `atob('')` می‌کرد → مسیر شکسته.
- `bootExactMathEditor(cb)` تنها فراخوانِ `exactMathEditorHtml()` بود و خودش
  **هیچ فراخوانی در کل فایل نداشت** (`grep` → صفر مورد فراخوان).
- `exactMathBooted` فقط دو انتساب باقی‌مانده دارد (بی‌ضرر)؛ عمداً دست نخورد تا
  ریسک صفر بماند.

حذف شد: تعریف ثابت + دو تابع. حجم asset: 6,180,662 → 6,178,881 بایت.

آنچه **دست نخورد** (تأیید با تست): `MATH_EDITOR_HTML` (ویرایشگر فرمولِ واقعی و
زنده)، `#mathEditorFrame`، `qimgStudioSrc` (استودیوی HTML — طبق تعهد V76.4 تا
پایان پورت می‌ماند) و هر ۹ پل بومی `__qmf*`.

راستی‌آزمایی: همهٔ ۱۶ بلوک `<script>` غیر-`text/plain` با `new Function()` در node
پارس شدند (۰ خطای نحوی)؛ verify PASS؛ شبیه‌سازی پین‌ها روی **همهٔ** `V7*Test.kt`
(درس ۵۱): ۳۳۹ پین، ۰ خطا.

verify: باند V76.8 (غیابِ سه نشانهٔ مرده + حضور ابزارهای زنده + وجود فایل تست) و
پین چنج‌لاگ به `V76.8:` به‌روز شد. تست: `V76_8AssetDeadCodeCleanupTest` (۳ تست).

**تصمیم باز (برای V76.9/پایان مسیر)**: تکلیف ابزارهای فقط-HTMLِ استودیو (OCR فارسی،
قطره‌چکان، لایهٔ پیشرفتهٔ اشیاء، فلش منحنی، فیلترهای اسکن کتاب، پایپ‌لاین OpenCV،
ZIP/PDF/Word، حالت تاریک) باید روشن شود، چون حذف `qimgStudioSrc` با تعهد
...
## ۳۱۰) V77.0 — فیلترهای اسکن کتاب + قطره‌چکان + لایهٔ اشیاء + فلش منحنی + حالت تاریک

CI برای V76.9 سبز شد (OCR فارسی کار کرد). کاربر از میان گزینه‌ها **«پنج مورد
مفید در یک پچ»** را انتخاب کرد و صریح گفت ZIP/PDF/Word اضافه نشود؛ OpenCV و
بنچمارک GPU هم کنار گذاشته شدند. هیچ کتابخانهٔ جدیدی اضافه نشد.

### فایل جدید `ExamImageStudioFilters.kt`
تمام منطق روی `IntArray`/دادهٔ خالص نوشته شد تا در تستِ معمولیِ JVM **واقعاً
اجرا** شود (نه فقط پینِ متنی). پوشش‌های `Bitmap` نازک‌اند.

### دو باگِ واقعی که شبیه‌سازی گرفت (پیش از CI)
1. **حذف سایه با `cells=12` کافی نبود** — پراکندگیِ کاغذ ۱۸ سطح ماند. با آزمونِ
   ۸/۱۲/۱۶/۲۴/۳۲ روی `cells=16` تثبیت شد (پراکندگی ۸، جوهر سالم).
2. **`despeckle` با میانهٔ سادهٔ ۳×۳ خطوطِ ۱ پیکسلی را کاملاً پاک می‌کرد** — فاجعه
   برای حروف نازکِ فارسی. بازنویسی شد به «حذفِ پیکسلِ منفرد»: پیکسل فقط وقتی
   اصلاح می‌شود که کمتر از `minNeighbors=2` همسایهٔ هم‌جنس داشته باشد. پس از
   بازنویسی: لکهٔ منفرد حذف، خطِ ۱px/۲px/۳px و خطِ **مورب** سالم، سوراخِ سفیدِ
   داخل جوهر پر. verify پینِ `minNeighbors` را قفل کرد تا رگرسیون ممکن نباشد.

### محدودیتِ صادقانه (ثبت‌شده در کامنت کد)
`flattenShadow` بر تقسیم بر کاغذِ محلی استوار است، پس **ناحیهٔ خاکستریِ بزرگ
(عکسِ تمام‌صفحه) سفید می‌شود**. آزمونِ «کفِ روشنایی» برای رفعش امتحان شد ولی
سایه‌زدایی را خراب می‌کرد (پراکندگی ۱۴→۵۰). چون این محدودیتِ ذاتیِ روش است،
فیلتر **پیش‌فرض خاموش** ماند و کاربر آگاهانه روشنش می‌کند.

### تغییرهای `ExamImageStudioCore.kt`
- `StudioShape` چهار فیلد گرفت: `locked`، `hidden`، `group`، `curve`.
- نوعِ جدید شکل `curve` در **هر دو** مسیر رندر (پیش‌نمایش زنده + `bakeShapes`)؛
  سرِ فلش از راستایِ آخرین پاره‌خطِ بزیه محاسبه می‌شود نه خطِ مستقیمِ دو سر.
  عمداً `quadraticBezierTo` (منسوخ، هشدار در لاگ CI) استفاده نشد.
- `encodeCropped`/`processAndEncode`/`prepareForOcr` پارامترهای `deshadow`/`denoise`
  گرفتند؛ فیلترها **قبل از** پختِ شکل‌ها اجرا می‌شوند تا رنگِ فلش/هایلایتر
  خاکستری نشود. OCR هم از همین فیلترها سود می‌برد.
- قفل/پنهان در hit-test رعایت می‌شود؛ هم‌گروه‌ها با هم جابه‌جا می‌شوند.
- `layerActionTargets`: اگر شکلِ انتخاب‌شده گروه دارد → اعضای گروه؛ وگرنه همهٔ
  آزادها (تا دکمه‌های هم‌ترازی روی انتخابِ تکی بی‌اثر نباشند).

### درسِ تکرارشده (بار سوم)
تغییرِ امضای `encodeCropped` **سه پینِ کهنه** ساخت (verify + تست V76.6 + تست
V76.7). verify فقط یکی را گرفت؛ **شبیه‌سازِ پین‌ها آن دوتای دیگر را گرفت**. هر دو
ابزار لازم‌اند. اجرای نهایی: **۳۷۱ پین، ۰ خطا**.

### راستی‌آزمایی
verify PASS (۲۰۸ فایل کاتلین) · ۳۷۱ پین ۰ خطا · منطقِ فیلترها/هندسه عیناً در
python شبیه‌سازی شد (سایه‌زدایی، لکه، خطِ نازک، برشِ خودکار، بزیه، ترتیب،
هم‌ترازی، توزیع — همه مطابق انتظار) · دروازهٔ import به فایل جدید گسترش یافت ·
تست جدید `V77_0StudioFiltersAndLayersTest` با **۱۷ تست** که منطق را واقعاً اجرا
می‌کنند.

پچ: V77_0_book_scan_eyedropper_layers_curve — بدون SQL.

## ۳۱۱) V77.0.1 — رفع تست قرمز (پینِ چندخطیِ امضای encodeCropped) + رفعِ باگِ خودِ شبیه‌ساز

CI برای V77.0: کامپایل **سبز**، ۷۵۳ تست اجرا، **۱ تست قرمز**:

```
V76_7StudioDrawToolsTest > shapes are baked into the output pipeline FAILED
    java.lang.AssertionError at V76_7StudioDrawToolsTest.kt:91
```

ریشه: پینِ خط ۹۱ امضای `encodeCropped` را به‌صورتِ «`shapes: ... = emptyList()`
بلافاصله قبل از `): Pair<String, Int>?`» قفل کرده بود. V77.0 دو پارامترِ
`deshadow`/`denoise` را **بین** این دو اضافه کرد، پس رشته دیگر پیدا نشد. خودِ
زنجیرهٔ پخت تغییری نکرده بود — فقط پین کهنه شد. پین به شکلِ فعلیِ امضا
به‌روزرسانی و به دو ادعای مستقل شکسته شد.

### باگِ ابزارِ خودم (مهم‌تر از خودِ فیکس)
در V77.0 من سه پینِ کهنه را گرفتم ولی **این یکی از دستم در رفت**. علت: الگوی
regex در `/tmp/simulate_reader_tests.py` فقط `assertTrue("..." in x)` را در
**یک خط** می‌دید. این پین به شکلِ
...
و پس از فیکس به شکلِ الحاقیِ `"a" + "b" + "c"` بود — هر دو نامرئی برای الگو.
شبیه‌ساز بازنویسی شد تا: (۱) `assertTrue(` چندخطی، (۲) رشته‌های **به‌هم‌چسبیدهٔ
با `+`**، و (۳) `"""..."""` چندخطی را هم پوشش دهد. شمارِ پین‌ها ۳۷۱ → **۳۷۴**
رفت (یعنی سه پین قبلاً اصلاً بررسی نمی‌شدند). تستِ منفی گرفته شد: با خراب‌کردنِ
عمدیِ یک جزء از پینِ الحاقی، شبیه‌ساز آن را گرفت.

**درس ۵۲ (دائمی)**: وقتی امضای یک تابع عوض می‌شود، پین‌هایی که **چند خط از
امضا** را با هم قفل کرده‌اند شکننده‌ترین‌اند. پینِ چندخطیِ امضا تا حد امکان به
...
## ۳۱۳) V91 — رفع ۶ ایرادِ پنجرهٔ پیش‌نمایش و بازیابیِ آزمون‌سازِ چاپی

پس از تستِ واقعیِ کاربر، شش ایرادِ آزمون‌سازِ چاپی رفع شد. همه در `exam_print.html`
(موتورِ WebView) یا `ExamHtmlPrintDialog.kt` (میزبانِ بومی) هستند.

### ۱) جابه‌جاییِ اشیاء محدود به همان سؤال شد
- در `initPreviewFigureEditing`، ارتفاعِ سلولِ سؤال هم اندازه‌گیری می‌شود
  (`parentH` در `measureParent`).
- شیءِ شناور (`fig-free`) به‌جای کفِ بازِ `-4000` حالا بینِ کف و سقفِ سلولِ
  خودش گیره می‌خورد: `y = Math.max(0, Math.min(maxY, y))` با
  `maxY = m.parentH - h - borderSafe`. دیگر شیء به سؤالِ بالا/پایین نمی‌رود.

### ۲ + ۳) حذفِ کادرِ سفیدِ هر شیء در پیش‌نمایش
- قانونِ جدید `#previewArea .interactive-figure .qmf-fig` با
  `background:transparent / box-shadow:none / padding:0 / border-radius:0 / margin:0`.
- کادرِ سفید (پس‌زمینه + سایه + padding) که اشیاء را از هم جدا و روی ستونِ
  بارم می‌انداخت برداشته شد؛ حالا اشیاء بدونِ همپوشانی کنارِ هم می‌نشینند و
  ستونِ بارم پوشیده نمی‌شود. (فقط در پیش‌نمایش؛ ویرایشگرِ آنلاین دست‌نخورده است.)

### ۴) دکمهٔ بازیابی حالا واقعاً کار می‌کند
- ریشه: `restoreAutosave` در JS هیچ مقداری برنمی‌گرداند، پس کال‌بکِ کاتلین
  `'ok'` نمی‌دید، پیامِ «بازیابی ناموفق بود.» می‌داد و فهرستِ بومی را تازه
  نمی‌کرد (سؤال‌ها فقط بعد از افزودنِ سؤالِ تازه ظاهر می‌شدند).
- حالا `restoreAutosave` روی موفقیت `'ok'` برمی‌گرداند و میزبان
  `cardsRefresh++` می‌زند تا سؤال‌های بازیابی‌شده همان لحظه در فهرستِ بومی
  بیایند، و `mirrorDraft()` آینهٔ بومی را هم‌گام می‌کند.

### ۵) پیامِ «آزمون بازیابی شد» بومی و درست شد
- در میزبانِ بومی، `qmfToast` داخلیِ JS صدا زده نمی‌شود و فقط پیامِ بومیِ
  «آزمون بازیابی شد ✓» می‌آید (پیش‌تر دو پیام پشتِ‌سرهم می‌آمد و آخرین پیام
  «بازیابی ناموفق بود.» بود). در مرورگر همان اعلانِ قدیمی می‌ماند.

### ۶) آزمون‌سازِ چاپی از همان ابتدا بومی باز می‌شود
- وقتی هنوز سؤالی ساخته نشده، به‌جای صفحهٔ خالیِ WebView یک حالتِ خالیِ بومی
  («هنوز سؤالی ساخته نشده است — از دکمهٔ + یک سؤال اضافه کنید») نمایش داده
  می‌شود؛ دیگر «اول WebView بعد بومی» دیده نمی‌شود.

### راستی‌آزمایی
- `compileDebugKotlin` + `compileDebugUnitTestKotlin`: **BUILD SUCCESSFUL**.
- `testDebugUnitTest`: همهٔ تست‌ها سبز (شامل تستِ جدید `V91_0PreviewRestoreFixTest`).
- `scripts/verify_native_final.py`: **PASS**.
- سینتکسِ ۱۷ اسکریپتِ درونِ `exam_print.html` با `node --check` بررسی شد: ۰ خطا.
...
## ۳۱۵) V93 — رفع ۵ گزارشِ جدیدِ کاربر از آزمون‌سازِ چاپی

پس از تستِ واقعیِ کاربر، پنج ایرادِ جدید رفع شد. فایل‌ها:
`exam_print.html` (موتورِ WebView) و `ExamHtmlPrintDialog.kt` (میزبانِ بومی).
SQL/Edge جدید ندارد.

### ۱) بازیابی با تأخیر سؤال‌ها را نشان می‌داد
- ریشه: `restoreAutosave` در یک نفس `renderAll()` (ساختِ کارت‌های ویرایشگرِ
  HTML + برگهٔ A4 پیش‌نمایش) را اجرا می‌کرد؛ پاسخِ `'ok'` فقط پس از این رندرِ
  سنگین به پلِ بومی می‌رسید و تازه آن‌وقت `cardsRefresh++` فهرستِ بومی را
  می‌خواند.
- حالا `questions`/`qIdCounter`/فیلدهای سربرگ همان لحظه بازیابی می‌شوند و
  `renderAll()` با `setTimeout(..., 60)` به چرخهٔ بعدی می‌رود؛ پاسخ فوراً
  برمی‌گردد و کارت‌های بومی بی‌تأخیر ظاهر می‌شوند.

### ۲) با زدنِ دکمهٔ چشم، هدر و دکمه‌های شناور محو نمی‌شدند
- در `ExamHtmlPrintDialog.kt` هدرِ سفیدِ بالا، ردیفِ سه‌دکمهٔ پایین (ذخیره/
  چاپ/چشم) و دکمهٔ `+` حالا در `AnimatedVisibility(visible = !previewOpen)`
  با `fadeIn/fadeOut` پیچیده شده‌اند؛ با باز شدنِ پیش‌نمایش همه محو می‌شوند
  تا برگهٔ A4 تمام‌صفحه دیده شود.

### ۳) جملهٔ «پیش‌نمایش آزمون» و آیکنِ چشم در هدرِ پیش‌نمایش
- در `openPreviewWindow` (exam_print.html) اسپنِ `<span>👁 پیش‌نمایش آزمون</span>`
  از `pwo-head` حذف شد؛ فقط کنترل‌های زوم و دکمهٔ بستن می‌مانند.

### ۴) لمس/انتخابِ اشیاء در پیش‌نمایش پرش داشت و حرکت آزاد نبود
- ریشه: در `clampToParent` سقفِ عمودیِ شیءِ شناور از «ارتفاعِ سلولِ اندازه‌گیری‌
  شده در آغازِ درگ» حساب می‌شد؛ چون شیءِ مطلق از جریانِ متن بیرون است، سلول
  کوتاه می‌ماند، سقف منفی می‌شد و شیء با نخستین حرکت به بالا می‌پرید و همان‌جا
  قفل می‌شد.
- حالا فقط کفِ صفر (`y = Math.max(0, y)`) می‌ماند و سلول با `minHeight` خودش
  همراهِ شیء رشد می‌کند؛ حرکتِ عمودی کاملاً آزاد است و شیء همچنان از سلولِ
  سؤالِ خودش بیرون نمی‌زند.

### ۵) چاپ با پیش‌نمایش یکی نبود
- ریشه: بلوکِ `qmf-print-box-clean-v92` شیءِ شناور را در چاپ با
  `position:relative` بازجریان‌سازی می‌کرد (وسط‌چین، تمام‌عرض)؛ پس جایِ شیء در
  چاپ با جایی که کاربر در پیش‌نمایش گذاشته بود فرق می‌کرد.
- حالا در چاپ شیءِ شناور `position:absolute` می‌ماند (همان مختصات، اندازه و
  لایهٔ پیش‌نمایش) و فقط کادرِ سفیدش حذف است تا روی ستونِ بارم سفید نکشد.

### راستی‌آزمایی
- `testDebugUnitTest`: ۱۰۵۸ تست سبز (تست‌های V89.6 و V91.0 به رفتارِ V93
  به‌روز شدند).
- `lintDebug`: BUILD SUCCESSFUL.
- `scripts/verify_native_final.py`: PASS (`FINAL_NATIVE_VERIFY=PASS`).
- سینتکسِ ۱۷ اسکریپتِ درونِ `exam_print.html` با `node --check`: ۰ خطا.
...
## ۳۱۷) V95 — دست‌نویس کار می‌کند + پنجرهٔ دکمهٔ «()» پهن و خوانا شد

دو گزارش کاربر پس از جایگزینیِ بایت‌به‌بایتِ ویرایشگر (V94). هر دو فقط در
`formula.html` رفع شد؛ تغییری در Kotlin لازم نبود.

### ۱) «بخش دست‌نویس کار نمی‌کند»
- مودالِ دست‌نویس (`hwModal` + بومِ `ink`) خودش درست بود؛ روی دستگاهِ کاربر
  با همان باگِ compositingِ شناخته‌شده (WebView شفاف + `backdrop-filter` +
  انیمیشن → مودال باز ولی paint نمی‌شود، همان «صفحهٔ سفیدِ» V55.2/V55.3)
  ممکن بود خالی دیده شود.
- `nativePaintFix` (فقط داخل برنامه) حالا `backdrop-filter` پس‌زمینهٔ مودال
  (`#mback`) و انیمیشنِ `#hwModal` را هم خنثی می‌کند؛ ظاهرِ مرورگرِ عادی
  دست‌نخورده است.
- `openHw` به‌صورت `window.mbHwOpen` بیرون داده شد و stub اولیهٔ دکمهٔ مداد
  در نبودِ override آن را صدا می‌زند (شبکهٔ ایمنی).

### ۲) «پنجرهٔ دکمهٔ () باریک است»
- منوی سه‌ستونیِ پرانتز (`mbParMenu`) زیر enforcement دستگاه‌محورِ V55.5/V55.8
  نبود (style مستقیم با px خالص که روی آن WebView جواب می‌دهد؛ نه CSS تزریقی
  با `min()`).
- حالا `mbParMenu` هم wrap شده و دکمه‌های `.mbv-parbtn` با style مستقیم
  بزرگ می‌شوند: `min-height:46px`، فونتِ برچسب `.8rem`، نماد `1.6rem`؛
  عرضِ منو تا `min(520px, 96vw)` باز می‌شود.
- CSS پایهٔ `.mbv-parbtn`/`.mbv-par-lab`/`.mbv-par-g` هم بزرگ‌تر شد تا در
  مرورگرِ عادی هم خوانا باشد.

### راستی‌آزمایی
- `scripts/verify_native_final.py`: PASS.
- `testDebugUnitTest`: ۱۰۶۲ تست، ۰ شکست (تستِ جدیدِ V95.1 با ۴ مورد).
- `lintDebug`: BUILD SUCCESSFUL.
- مرورگرِ هدلس (Chromium + تزریقِ پلِ Native): مودالِ دست‌نویس باز می‌شود،
  خطِ رسم ثبت و شناسایی می‌شود؛ منوی پرانتز ۳۷۴px با ستون‌های ۱۱۱px و
  دکمه‌های ۴۶px (enforcement مستقیم اعمال شد).
...
## ۳۲۰) V98 — حذف دکمهٔ وب‌ویوی آزمون‌ساز چاپی، سیاه شدن نام پرانتزها، اصلاح وسط‌چین راهنمای دست‌نویس، رندرینگ متن فارسی فرمول‌ها، تنظیمات سربرگ در ایجاد آزمون و استانداردسازی تحویل با `apply_vXX.py`

شش اصلاح و بهبود نهایی:
...
۶) جایگزینی قطعی روش تحویل سنتی `git apply` با اسکریپت‌های مستقل و خودکار پایتون (`apply_vXX.py`) برای تضمین اعمال ۱۰۰٪ بدون تعارض و خطای گیت.
...
- `apply_v98.py`: ایجاد اسکریپت خود-استخراج‌گر و استقرار دهندهٔ خودکار نسخهٔ V98.
...
### راستی‌آزمایی
- `testDebugUnitTest`: ۱۰۷۷ تست در ۲۰۶ کلاس، ۰ شکست، ۰ خطا.
- `FINAL_NATIVE_VERIFY=PASS`.
```text
SQL/Edge/Secret/Dependency جدید: ندارد
```

تحویل: اسکریپت خودکار apply_v98.py — بدون SQL.

## ۳۲۱) V99 — پیش‌نمایش مستقیم برگه A4، ثبات خطوط متن هنگام لمس اشیاء و تشخیص خودکار ارتفاع کادر سؤال

سه اصلاح و بازسازی اساسی:
۱) **پیش‌نمایش مستقیم و خالص برگهٔ A4:** با لمس آیکون چشم در «آزمون جدید» بخش چاپ، پیش‌نمایش بدون باز شدن یا فلش زدن پنجرهٔ آزمون‌ساز چاپی قدیمی به‌صورت یک دیالوگ تمام‌صفحه و تمیز برگهٔ A4 چاپی همراه با دکمه‌های زوم و بستن باز می‌شود؛ در حالت چاپ مستقیم (دانش‌آموز/پاسخ‌نامه) نیز اجرای چاپ به‌صورت پس‌زمینه و مستقیم انجام می‌شود.
۲) **ثبات خطوط متن و عدم جابه‌جایی ناخواسته:** در پیش‌نمایش، لمس ساده یا انتخاب اشیاء (تصاویر، نمودارها، فرمول‌ها) دیگر شیء را از جریان متن خارج نکرده و خطوط متن ثابت می‌مانند. جابه‌جایی و شناور شدن شیء فقط با درگ صریح کاربر (drag > 5px) انجام می‌شود.
۳) **تشخیص و تنظیم خودکار ارتفاع کادر متن سؤال:** هنگام درج اشیاء، فرمول‌ها یا متون چندخطی در آزمون جدید، ارتفاع کادر متن سؤال در پیش‌نمایش به‌صورت پویا و خودکار اندازه‌گیری و تطبیق داده می‌شود تا هیچ بخشی از متن یا فرمول بریده نشود.

### تغییرات فایل‌ها:
- `app/src/main/assets/print/exam_print.html`: اصلاح مدیریت اشاره‌گر در `initPreviewFigureEditing`، حفظ جریان درون‌متنی اشیاء تا زمان درگ صریح، و تطبیق خودکار ارتفاع کادر متن سؤال.
- `app/src/main/java/ir/exam/app/ui/printing/ExamHtmlPrintDialog.kt`: بازطراحی حالت پیش‌نمایش مستقیم (`initialPreview`) و چاپ مستقیم بدون نوار ابزار یا کارت‌های قدیمی.
- `app/src/test/java/ir/exam/app/ui/app/V97_PrintAndFormulaFixesTest.kt`: افزودن تست‌های اعتبارسنجی V99 برای عدم تغییر خطوط با لمس و نمایش مستقیم پیش‌نمایش.
- `app/src/test/java/ir/exam/app/ui/app/V89_6LivePreviewCssTest.kt`: هماهنگی تست درگ اشیاء.
- `scripts/verify_native_final.py`: به‌روزرسانی اعتبارسنجی‌ها برای رفتار پیش‌نمایش و اشیاء.

### راستی‌آزمایی
- `testDebugUnitTest`: ۱۰۷۹ تست در ۲۰۶ کلاس، ۰ شکست، ۰ خطا.
- `FINAL_NATIVE_VERIFY=PASS`.
```text
SQL/Edge/Secret/Dependency جدید: ندارد
```
...
## ۳۲۳) V99.1b — سازگارسازی با CI: ۴ پینِ تستِ قدیمی

اجرای CI روی V99.1 (۱۰۸۷ تست) ۴ شکست نشان داد — همهٔ آن‌ها پین‌های تست‌های قدیمی بودند که با رفتارِ تازه در تضاد بودند (در sandbox فقط ۴ فایلِ تست بررسی شده بود، نه کلِ ریپازیتوری):
- `V89_5PreviewCoverTest`: پینِ شرطِ کارت‌ها → با `initialPrintMode == null` به‌روز شد.
- `V89_6LivePreviewCssTest`: پینِ رشد minHeight → به‌جای آن نبودنِ آن + وجود `syncFigFlowSlot` پین شد.
- `V62_7PrintCenterStudentPreviewTest`: «چاپ با کلید» حالا مشروع است (برچسبِ منوی چاپِ V99.1) → پین به «فقط در منو، نه روی کارت» تغییر کرد.
...
## ۳۳۰) V108 — میکروفون گفتار به متن (فارسی/انگلیسی) + تبدیل هوشمند به عدد/نماد/فرمول

### درخواست
کنار آیکن تصویر یک میکروفون که صدا (فارسی و انگلیسی) را به متن تبدیل کند و به‌طور هوشمند با کمک ویرایشگر فرمول، نماد/عدد/فرمول را تایپ کند؛ هم در بخش چاپ آزمون و هم آنلاین.

### طراحی
- **`core/speech/SpeechMathConverter.kt`** (بدون وابستگی اندروید، تست JUnit واقعی): `convert(spoken): Result(text, containsFormula)`.
  - اعداد گفتاری فا/انگ (با «و»/and، هزار/میلیون، ممیز/point) → رقم.
  - نمادهای متنی: درصد/percent، درجه، علامت سؤال، ویرگول، دونقطه، پرانتز، سر خط، جای خالی.
  - اگر جمله نشانهٔ ریاضی داشته باشد (`looksMathematical`: توان، رادیکال، کسر/روی، انتگرال، مجموع، حد، مشتق، سینوس…، برابر، بزرگ‌تر…) → LaTeX داخل `$…$`: `^{}`، `\sqrt{}`، `\frac{}{}`، `\int_{}^{}`، `\sum`، `\lim_{x \to \infty}`، `\sin`…، حروف یونانی، `\times \div = \neq \geq \leq \approx`، متغیرهای گفتاری (ایکس/ایگرگ/زد/آر…). ترتیب: توابع/یونانی/متغیر → عملگرها → ساختارها (اتم = واژه یا پرانتز).
- **`ui/speech/SpeechToTextButton.kt`**: IconButton میکروفون → منوی زبان (fa-IR / en-US) → مجوز RECORD_AUDIO (runtime) → `SpeechRecognizer` سیستم با نتایج جزئی (دیالوگ «در حال شنیدن…») → `deliver()`: فرمول → `onFormula(tex)`، متن → `onText`. خطاهای رایج فارسی‌سازی شده‌اند؛ recognizer در onDispose آزاد می‌شود.
- **`QuestionMediaEditor.kt`**: پارامترهای اختیاری `onSpeechText/onSpeechFormula`؛ دکمه کنار دوربین.
- **`ExamBuilderScreen.kt`**: متن گفتاری به انتهای متن سؤال (`updateText` + `setValue`); فرمول: متن سؤال + `$tex$` با انتخابِ همان بازه در `FormulaHostDialog` (ویرایشگر فرمول با همان فرمول باز می‌شود؛ کاربر بازبینی/درج می‌کند). چون کارت سؤال در هر دو حالت `printMode` و آنلاین یکی است، در هر دو بخش فعال است.
- **Manifest**: `RECORD_AUDIO`.

### راستی‌آزمایی
- `SpeechMathConverterTest` (۳ تست، ۱۷ assert) با kotlinc 2.0.21 + JUnit 4.13.2 در sandbox اجرا شد: **OK (3 tests)**. نمونه‌ها: «بیست و پنج درصد دانش آموزان»→`25٪ دانش آموزان`؛ «ایکس به توان دو به علاوه سه ایکس مساوی صفر»→`$x^{2} + 3 x = 0$`؛ «پی ضرب در آر به توان دو»→`$\pi \times r^{2}$`؛ «integral from zero to one x»→`$\int_{0}^{1} x$`.
- `V108_SpeechToTextTest` (پین سیم‌کشی). verify PASS، diff --check تمیز.
- روی دستگاه: نیاز به Google app/سرویس گفتار؛ فارسی معمولاً آنلاین است. اشکال‌های شناخته‌شده: عبارت‌های ریاضی طولانی/تودرتو ممکن است ناقص تبدیل شوند — به همین دلیل فرمول همیشه در ویرایشگر باز می‌شود نه درج کور.
...
## ۳۳۲) V110 — درج مستقیم فرمول گفتاری، پنجرهٔ آرام میکروفون، فضای پاسخ جای‌خالی/عددی

### درخواست‌ها و تغییرات
1. **فرمول گفتاری بدون بازکردن ویرایشگر:** `ExamBuilderScreen.onSpeechFormula` حالا `$tex$ ` را به انتهای متن سؤال اضافه می‌کند (`updateText` + `setValue`)؛ کاربر با لمس فرمول (لمس اول انتخاب، دوم ویرایش — مسیر موجود `QuestionTextWebSection`) ویرایشگر را باز می‌کند. متن راهنمای پنجره به‌روز شد.
2. **پنجرهٔ آرام:** `SpeechToTextButton` — `onRmsChanged` خالی، `LinearProgressIndicator` و `CircularProgressIndicator` حذف؛ سکوت/NO_MATCH/SPEECH_TIMEOUT بی‌صدا فقط قطعهٔ بعدی را (۱۵۰ms بعد) شروع می‌کند و هیچ متنی تغییر نمی‌دهد؛ فقط یک سطر وضعیتِ ثابت («در حال شنیدن…» / «در حال تبدیل…» / «متوقف شد…») و پیام شبکه در صورت خطا.
3. **آزمون چاپی:** `QuestionType.FILL_BLANK -> if (!printMode)` و `NUMERIC -> if (!printMode)` (کلیدهای تصحیح خودکار مخصوص آنلاین). داده‌ها حفظ می‌شوند؛ فقط UI پنهان است.
4. **فضای پاسخ برای جای‌خالی/عددی:** `PrintAnswerSpaceControls` برای ESSAY/FILL_BLANK/NUMERIC؛ `ExamHtmlPrintPayload` برای `numeric`/`fill` هم `answerLines` واقعی را می‌فرستد (قبلاً ثابت ۱)؛ رندرر `answerBlock` دیگر عددی را به ۱ سطر و جای‌خالی را به حداقل ۱ محدود نمی‌کند (۰ سطر = بدون فضای پاسخ).

### تست
- پین‌ها: `V108_SpeechToTextTest` (درج مستقیم، بدون نوار RMS) و `V107_PrintBuilderPolishTest` (پنهان‌شدن کلیدها در حالت چاپی) به‌روز؛ verify PASS؛ diff --check تمیز.
...
## ۳۳۴) V112 — میکروفون با پنجرهٔ استاندارد گفتار گوگل (پایان حلقهٔ SpeechRecognizer)

### گزارش کاربر
«پنجره تند تند رفرش می‌شود، آلارم می‌دهد و تشخیص بسیار ضعیف است» (حتی بعد از V111).

### ریشهٔ واقعی
معماری V109–V111 («شنیدن پیوسته» با `SpeechRecognizer` و `startListening` مجدد برای هر قطعه) ذاتاً همین رفتار را دارد: هر `startListening` بوقِ سیستمیِ شروع و هر پایان/سکوت بوقِ پایان می‌زند (همان «آلارم»)، هر قطعه پنجره را تکان می‌دهد، و موتور با قطعه‌های کوتاهِ بدون زمینه نتایج ضعیف می‌دهد. با پنهان‌کردن نوار/وضعیت (V110/V111) درست نمی‌شد؛ باید معماری عوض می‌شد.

### راه‌حل
`SpeechToTextButton.kt` بازنویسی شد روی **`RecognizerIntent.ACTION_RECOGNIZE_SPEECH` + `StartActivityForResult`** (پنجرهٔ خودِ گوگل):
- یک بوق، UI آشنا، مجوز میکروفون توسط سرویس گوگل، کیفیت کامل (`EXTRA_MAX_RESULTS=5` → `pickBest` → `convertPlain`).
- بعد از نتیجه، پنجرهٔ ویرایش برنامه با کادر متن؛ «ضبط بیشتر» = یک نوبت دیگر و افزودن به انتهای متن؛ «درج»/«انصراف». لغو پنجرهٔ گوگل وقتی متنی نیست، همه چیز را می‌بندد.
- هیچ `SpeechRecognizer`/`RecognitionListener`/state لحظه‌ای در برنامه نیست. مجوز `RECORD_AUDIO` در مانیفست می‌ماند (بی‌ضرر).
- تصمیم: «شنیدن پیوسته» (V109) کنار گذاشته شد — با موتور گوگل بدون بوق ممکن نیست؛ به‌جایش «ضبط بیشتر».
- تست `V108_SpeechToTextTest` بازنویسی (پین پنجرهٔ سیستمی، نبودِ حلقه).
...
## ۳۳۶) V113.1 — هات‌فیکس تست CI

`V20InteractionPolishTest` پین منفی دارد: `assertFalse("Text(\"درس:" in dashboard)` روی `TeacherDashboardScreen.kt`. کارت‌های پنجرهٔ «آزمون‌های چاپی» (V113) این عبارت را داشتند → CI شکست. برچسب به `"${subject.ifBlank { "بدون درس" }} · n سؤال"` تغییر کرد (هم‌قالب کارت‌های آنلاین).

**درس برای نسخه‌های بعد:** پیش از تحویل، علاوه بر پین‌های مثبت، پین‌های `assertFalse(... in file)` تست‌ها را هم روی فایل‌های تغییرکرده بررسی کنید.
...
## ۳۳۹) V115 — هدر واقعاً ثابت + پیش‌نمایش چندبرگی A4

### چرا هدر V114 ثابت نبود
`position:fixed` روی WebView اندروید با zoom/اسکرول افقی (حالت اندازهٔ واقعی) غیرقابل‌اعتماد است و هدر با صفحه می‌رفت. راه‌حل: **خودِ سند اسکرول نمی‌کند** (`html,body{height:100%;overflow:hidden}`, body = flex column)، `#screenChrome` عنصر عادی بالای ستون و `#printSurface{flex:1;overflow:auto}` تنها اسکرول‌کننده است. همهٔ `window.scrollTo` → `surface.scrollTop/Left`. مقیاس fit از `surface.clientWidth`. در `exam-print-mode` (چاپ) با کلاس `exam-print-mode-root` روی `<html>` همه‌چیز به جریان عادی برمی‌گردد (میزبان Kotlin هم همین کلاس را برای چاپ مستقیم اضافه می‌کند).

### پیش‌نمایش چندبرگی (`paginatePreview()` در render)
- برگهٔ منطقی همچنان **یک** `#sheet` است (مختصات اشیاء شناور/جداکننده/انتخاب متن دست‌نخورده)؛ در حالت `preview-open` کلاس `paged` می‌گیرد: پس‌زمینه/کادر خودش شفاف و برای هر ۲۹۷mm یک `.page-frame` (absolute، کادر ۲px، سایه، برچسب «برگهٔ n از m») زیر محتوا کشیده می‌شود. فاصلهٔ بین برگه‌ها `PAGE_GAP=14px`.
- شکستن: جدول سؤال‌ها سطر به سطر بررسی می‌شود؛ اولین سطری که از حدِ برگهٔ جاری (۲۹۷mm − padding) می‌گذرد به همراه بقیهٔ سطرها به یک `<table>` تازه با همان `colgroup/thead` می‌رود و یک `.page-spacer` قبل از آن، شروعش را روی padding بالای برگهٔ بعد می‌گذارد. سطرهای بلندتر از یک برگه شکسته نمی‌شوند. حلقه تا ۶۰ برگه.
- محاسبات با `offsetTop/offsetHeight` (بدون transform) → در fit-page هم درست است.
- در `exam-print-mode` هیچ‌کدام اجرا/نمایش نمی‌شود؛ چاپ همان مسیر قبلی (`break-inside:avoid`) است.
- `verify_native_final.py`: سقف حجم رندرر ۱۰۰KB → ۱۴۰KB (V114+V115).

### تست
Chrome headless (puppeteer) با ۳۰ سؤال تشریحی ۳ سطری، viewport 412×820: ۷ برگه، هیچ سطری روی مرز برگه نیست (`cross=0`)، هدر بعد از اسکرول در `top=0`، در اندازهٔ واقعی `scrollWidth=794=پهنای برگه` و سند بدون overflow افقی. (اسکرین‌شات‌ها حذف شدند؛ اسکریپت تست در ورک‌اسپیس نگه داشته نشد.)

```text
SQL/Edge/Secret/Dependency جدید: ندارد
تحویل: apply_v115.py (§۱۱)
شمارهٔ نسخهٔ بعدی: V116
```

---

## ۳۴۰) V115.1 — هات‌فیکس تست CI

`ExamPrintRendererContractTest` جدا از `verify_native_final.py` سقف `< 100_000L` بایت برای رندرر داشت؛ در V115 فقط verify به‌روز شده بود. تست هم به ۱۴۰KB رسید. **درس:** سقف حجم رندرر در دو جا پین شده است (scripts + test) — هر دو را با هم تغییر دهید.
...
## ۳۴۳. V117 — صفحه‌بندیِ واقعی (چاپ = پیش‌نمایش)، حذفِ دکمهٔ زوم، دیلیمترهای عینِ ویرایشگر

**خواستهٔ کاربر:** (۱) دکمهٔ «کل صفحه/اندازهٔ واقعی» حذف؛ پیش‌فرض اندازهٔ واقعی با
کمترین زوم، زومِ دستی آزاد. (۲) چاپ دقیقاً همان پیش‌نمایش باشد (۲ برگه پیش‌نمایش ≠ ۳
برگه چاپ). (۳) کادربندی حرفه‌ای: سطر/متن/بارم در انتهای برگه روی کادر قطع و در
برگهٔ بعد ادامه یابد. (۴) دیلیمترها عیناً مثل ویرایشگر فرمول.

**(۱) زوم:** `<meta viewport width=830>` (برگه + حاشیه) و `loadWithOverviewMode=true`
→ WebView در شروع کلِ برگه را در پهنای صفحه نشان می‌دهد (کمترین زوم) و زومِ
دوانگشتی آزاد است. `fitToggle`/`applyFit`/`fit-mode`/`real-mode`/`toggleFit` حذف
شدند؛ تست V107 به‌روز شد (اکنون نبودشان را می‌سنجد).

**(۲)+(۳) صفحه‌بندیِ واقعی:** تا V116 «برگهٔ منطقی» یکی بود، قاب‌های A4 فقط نمایشی
بودند و چاپ را مرورگر با `@page margin:5mm` و CSSهای متفاوتِ `@media print`
(padding/font کوچک‌تر) صفحه‌بندی می‌کرد → تعدادِ صفحه‌ها فرق داشت. حالا:
`render()` بلوک‌ها را می‌سازد و `paginate(flow)` آن‌ها را داخلِ برگه‌های واقعی
`.a4-page` (۲۱۰×۲۹۷mm، padding 5mm) با کادرِ `.a4-frame` (`overflow:hidden`،
border 2px، padding 6mm) می‌چیند. جدولِ سؤال‌ها سطر‌به‌سطر اضافه می‌شود؛ اگر سطر از
کادر بیرون بزند: اگر کمتر از `MIN_CUT_PX=30` جا داشته باشد کلِ سطر به برگهٔ بعد
می‌رود، وگرنه همان‌جا قطع می‌شود و در برگهٔ بعد زیرِ سرستون (`.table-head-only`)
یک `.row-continue` با کپیِ سطر (`.row-part`) و `margin-top` منفی به اندازهٔ بخشِ
چاپ‌شده می‌آید (چند برگه هم پشتیبانی می‌شود). چاپ همین DOM را چاپ می‌کند:
`@page margin:0`، هر `.a4-page` با `break-after:page` و ارتفاع `296.4mm` (تا
گردکردن، برگهٔ خالی نسازد). CSSهای print که چیدمان را عوض می‌کردند حذف شدند.
Puppeteer: ۱۴ سؤال → ۴ برگه پیش‌نمایش، PDF چاپ هم ۴ صفحه.
اشیاء شناور: مختصات `x,y` همچنان نسبت به کلِ پشتهٔ برگه‌ها (`#sheet`) ذخیره
می‌شود (snapshot سازگار)، ولی `applyFigureLayout` شیء را داخلِ `.a4-frame`ِ همان
برگه می‌نشاند تا در چاپ روی همان برگه بیاید (تست: y=1400 → برگهٔ ۲، همان مختصات).

**(۴) دیلیمترها:** `NativeMathSvgRenderer` حالا دقیقاً از همان جدولِ
`MB_DELIM_SHAPE` ویرایشگر (formula.html ~L7950) استفاده می‌کند: هر دیلیمتر از
قطعه‌های c (سرِ ثابت .3em)، m (نوکِ میانی .34em) و b (بازوی کشسان) در viewBox
۱۰×۱۰ ساخته می‌شود، fill:none با `vector-effect:non-scaling-stroke`، بسته‌ها
آینهٔ افقی. پهنا ثابت (`delimiterWidth`) مثل ویرایشگر. پوششِ (، [، {، ⌈، ⌊،
⟨، |، ‖. تست `NativeMathSvgRendererTest` (۹) سبز.
...
## ۳۴۸. V120 — بازبینیِ کد‌به‌کدِ پیش‌نمایش: سؤال‌های بعد از سطرِ بریده‌شده گم می‌شدند
...
## ۳۴۹. V124 — موتور پیش‌نمایش/چاپ PGS (آزمون‌ساز v20) در برنامه
...
**نکتهٔ نسخه‌گذاری (مهم برای ادامه):** مخزنِ GitHub در فاصلهٔ کارِ من چهار کامیتِ دیگر با شماره‌های
V120–V123 گرفت (رفع ۱۰ باگ، مکث زمان‌سنج، ترازِ پاراگرافی + پنجرهٔ «کادر»/PrintBoxStyleStore، خطِ آبیِ
سؤال + حذفِ کادرِ آزادِ تصاویر) و بیلدِ CI با نصبِ اولِ من (روی وضعیتِ قدیمی) از verify_native_final.py
رد شد. این نسخه به‌عنوان **V124** روی e4c27dc (سرِ مخزن) بازآمیزی شد: هر دو V120 (گارد مستقلِ حلقه +
...
**V125.1 (رفع خطای CI):** بیلد با `Unclosed comment` در ExamHtmlPrintDialog.kt رد شد؛ علت: عبارتِ
`print/web/*.css` داخلِ KDoc — در Kotlin کامنت‌ها **تودرتو** هستند و `/*` داخلِ کامنت، کامنتِ جدید باز
...
## ۳۵۲. V127 — پنل 📐 بریده، بازه/تعداد نسخه در دیالوگ چاپ، حذف امضای دبیر/مدیر

**گزارش کاربر:** ۱) پنل 📐 «تنظیمات صفحه» روی گوشی باز می‌شود ولی بریده/ناقص است؛ ۲) «بازهٔ صفحات» و
«تعداد نسخه»ی دیالوگ چاپ نیستند؛ ۳) سطرِ «نام و امضای دبیر / نام و امضای مدیر» از پیش‌نمایش/چاپ حذف شود.
کاربر خواست بازه/نسخه در همان دیالوگِ «چاپ آزمون» وب باشد.

**۱) ریشهٔ بریدگی پنل — viewport:** WebView با `useWideViewPort=true` + `loadWithOverviewMode=true` (تنظیم
V117 برای رندررِ قبلی) کل سند را zoom-out می‌کند؛ پنل‌های `position:fixed` موتورِ وب (`#pgsPageSetup`,
`#pgsPrintDlg`) نسبت به پنجرهٔ مجازیِ بزرگ چیده می‌شدند و media queryِ موبایلِ PGS (`max-width:820px` →
bottom-sheet تمام‌عرض) فعال نمی‌شد. موتورِ PGS خودش برگه را با transform در پهنای صفحه جا می‌دهد
(`pgsFit('width')`) و پینچِ خودش را دارد، پس overviewMode لازم نیست. اصلاح: `loadWithOverviewMode = false`
(متاتگِ `width=device-width` سند اعمال می‌شود). در شبیه‌ساز ۴۱۲px: پنل به‌صورت bottom-sheet کامل با
دکمهٔ «بستن ✕» و اسکرول داخلی.

**۲) بازه/نسخه:** پیش‌تر webhost دو سطرِ دیالوگ را با CSS پنهان می‌کرد. حالا `pgsDoPrintFromDlg` (override در
webhost.js) `rangeKind/rangeText/current/copies` را می‌خواند و به `requestPrint(mode, opts)` می‌دهد؛ پس از
`beforeprint` (ساختِ `#pgsPrintRoot`) تابعِ `applyRangeAndCopies` برگه‌های خارج از بازه را حذف و بقیه را به
تعدادِ نسخه clone می‌کند. **نکته:** بازه پس از ساختِ برگه‌های نهایی حل می‌شود، چون نسخهٔ استاد برگهٔ کلید
اضافه دارد (اول بر اساس تعدادِ برگه‌های دانشجو clip می‌شد → «۲-۳» فقط ۲ می‌داد). اعداد فارسی پذیرفته
می‌شود؛ بازهٔ نامعتبر → toast. تنها سطرِ پنهان‌شده راهنمای «Save as PDF» است. آزمون: استاد، «۲-۳»، ۲ نسخه →
`#pgsPrintRoot` = برگه‌های [2,3,2,3].

**۳) امضا:** متن از `OfficialExamPrintable.footerNote` (پیش‌فرضِ Kotlin: «نام و امضای دبیر: … مدیر:») می‌آمد و
webhost آن را در `opt_footerText` می‌گذاشت. پیش‌فرض خالی شد؛ موتورِ وب پاورقیِ خودش («موفق باشید – استاد –
واحد») را می‌گذارد. (OfficialPdfPrintAdapter فقط برای کارنامه است و footerNote خودش را دارد.)

verify/تست‌ها: `loadWithOverviewMode = false`, `applyRangeAndCopies`, نبودِ «نام و امضای دبیر:» در مدل، و
امضای جدید `requestPrint(mode, opts)`. نسخهٔ بعدی: V128، بند ۳۵۳.

**V127.1 — پنل 📐 «در پایین و یک‌سطری» باز می‌شد:** پس از V127 پنل در جای درست (پایین) بود ولی فقط عنوانش
دیده می‌شد. علت: ارتفاعِ آن در pgs_style.css با `max-height:82vh/82dvh` (موبایل) و `calc(100vh - 96px)`
حساب می‌شود؛ در WebViewِ اندروید داخلِ Dialogِ Compose مقدارِ vh/dvh قابل‌اعتماد نیست و تقریباً صفر شد
(در Chromium دسکتاپ/شبیه‌ساز درست بود — به همین دلیل در تست دیده نشد). اصلاح در webhost.css، بدونِ هیچ vh:
`#pgsPageSetup{position:fixed;top:60px;bottom:0;left:0;right:0;max-height:none;overflow-y:auto}` (بین نوارِ
بالا و کفِ صفحه کشیده می‌شود، محتوا داخلش اسکرول می‌خورد)، عنوان sticky با دکمهٔ «✕ بستن» بزرگ‌تر، فرم
...
**V128.1 — کاربر: «این دکمه‌ها در هدر پیش‌نمایش نیستند».** در شبیه‌ساز نوار ساخته می‌شد (فقط از `showPreview`).
برای اطمینان سه مسیرِ ساخت اضافه شد: (۱) wrap روی `window.openPreviewWindow` (هر بازشدنِ بیننده، از هر
...
**V130.1 — کاربر: «سؤالِ بلندتر از یک صفحه، ادامه‌اش به صفحهٔ بعد نمی‌رود و محو می‌شود».** ریشه در خودِ موتورِ
PGS (pgs_engine.js `splitOversizeRow`): تکه‌های سلولِ سؤال (`movableKids`) یکی‌یکی به سطرِ ادامه (`tr.q-cont`) در
صفحهٔ بعد می‌رفتند، اما وقتی فقط یک تکه می‌ماند (خودِ `.q-rich-content`) و answer-space نبود → `break` و clip.
اصلاح: `richCanSplit` / `splitRichPiece` — از انتهای `.q-rich-content` هر بار ۶ کلمه از آخرین تکهٔ `.txt`
(با `data-off` جدید = off + طولِ سر، تا نگاشتِ انتخاب→آفستِ V128 روی صفحهٔ ادامه درست بماند) یا یک گرهٔ کامل
(فرمول/شکل) به `.q-rich-content`ِ ادامه منتقل می‌شود؛ guard تا ۳۰۰۰ گام. در `placeRows` هم اگر سطر حتی در صفحهٔ
...
## §۳۵۸ — V133: فرمول‌ها عینِ ویرایشگر (جعبهٔ متن و پیش‌نمایش)، مرتب‌سازی ابزارهای استودیو، ترازِ متن در پیش‌نمایش

### ۱) فرمول در جعبهٔ متنِ بیلدر — `core/math/NativeMathSvgRenderer.kt` + `NativeMathAst.kt`
- مسیرِ رندرِ جعبهٔ متن بومی است (SVG کاتلین، بدون WebView). با هارنسِ kotlinc (۲۵ حالت: کسر، رادیکال، اندیس/توان، ماتریس، حد، انتگرال، …) با ویرایشگرِ فرمول (`formula.html`) مقایسهٔ تصویری شد.
- اصلاح‌ها: توان/اندیس با مقیاس .72 (قبلاً .58) و جای درست (توان کمی بالاتر از سقف پایه، اندیس کمی زیر کرسی، فاصلهٔ افقی .02em)؛ کسر: صورت/مخرج .92 اندازهٔ متن (قبلاً .76) و خط کسر با پدینگ .28em؛ رادیکال عینِ `mbFitSurd` ویرایشگر (جعبهٔ √ به عرضِ نصفِ ارتفاع، مسیر ۳/۴۷→۱۳/۷۸→۳۴/۰، ضخامت ۱٫۶، سرِ گرد؛ اندیسِ ریشه .6em روی علامت)؛ حداقل عرض نماد از .72em به .50em (فاصلهٔ ناخواستهٔ بین حروف حذف شد)؛ عرض حروف باریک (i j l t f r) و پهن (m w) جدا؛ `-` به منهای واقعی (U+2212) و عرض عملگرهای + − = ± .78em.
- پارسر: `\,` `\:` `\;` → فاصلهٔ نازک (U+2009) و `\!` → خالی (قبلاً □ چاپ می‌شد).
- خط کسر و خط بالای رادیکال از قبل تمام‌عرض بودند (بررسی شد).

### ۲) فرمول در پیش‌نمایش — `assets/print/web/math_host.js` + `webhost.js`
- مقایسهٔ puppeteer پیش‌نمایش با ویرایشگر: تنها تفاوت، پرانتز/کروشهٔ ماتریس‌ها بود (`( )` سادهٔ کوچک). حالا `pmatrix/bmatrix/Bmatrix/vmatrix/Vmatrix` داخل `.mparbox.mmatrix-box` با همان دلیمترهای SVG کشسانِ میزبان (`window.__hostDelimHtml` که `installDelimOverride` در webhost.js ثبت می‌کند؛ اگر نبود، گلیفِ مقیاس‌شونده) رندر می‌شوند و هم‌قدِ ماتریس‌اند.
- هشدار: `math_host.js` رشتهٔ عظیم `MATH_EDITOR_HTML` را هم دارد؛ `str.replace` روی کل فایل ممکن است داخل آن رشته هم عوض کند (این بار پیش آمد و JS خراب شد) — همیشه `node --check` بگیرید و جایگزینی را به قبل از `const MATH_EDITOR_HTML` محدود کنید.

### ۳) ترازِ متن در پیش‌نمایش — `webhost.js` / `webhost.css`
- زنجیرهٔ کاتلین (`AlignSpan`، `alignSpans` در payload/snapshot/codec) از قبل کامل بود؛ لایهٔ وب اضافه شد: `toWebQuestion` → `q.__aligns` (`cleanAligns`)؛ رندر: وقتی ترازی هست، هر پاراگراف (بین دو `\n`؛ خطِ جدیدِ داخل توکن شکل شمرده نمی‌شود) داخل `<div class="q-para" style="text-align:…">` می‌نشیند و آفست‌های `.txt` تغییر نمی‌کند (انتخاب/قالب‌بندی V128 سالم می‌ماند).
- نوار قالب‌بندی: چهار دکمهٔ SVG (`hf-al`, `data-fmt="align-right|center|left|justify"`) بین «فونت» و «پاک». روی پاراگرافِ دربرگیرندهٔ انتخاب اعمال می‌شود؛ تکرارِ همان تراز → برگشت به پیش‌فرض؛ با «همه» روی کلِ متن همهٔ سؤال‌ها. `formatSelection('align-left')` یا `('align','left')` هم از بومی قابل فراخوانی است. `snapshot()` حالا `alignSpans` می‌دهد و VM با `decodeSnapshotAlignSpans` ذخیره می‌کند.

### ۴) استودیوی تصویر — `ui/printing/ExamImageStudioCore.kt`
- ده ردیفِ درهمِ ابزار زیر یک نوارِ بخش‌ها (`studioTab`: «تصویر و برش / طراحی و علامت / بهبود و OCR / صاف‌سازی») رفت؛ فقط ابزارهای بخشِ فعال دیده می‌شوند و عوض‌کردنِ بخش حالت‌های ناسازگار (`splitMode/perspMode/drawMode`) را می‌بندد. `ToolChip` جمع‌وجورتر (ارتفاع ۳۶dp، پدینگ ۱۲/۶). متنِ دکمه‌ها عمداً عوض نشد (پین‌های V76_7StudioDrawToolsTest / ImageStudioEraserAndAbiTest).
- ویرایشگر سادهٔ تصویر (`InteractiveImageEditorDialog`) از قبل مرتب بود (۳ آیکن + تأیید/حجم/انصراف) و دست نخورد.
...
### V135.3 — تست V135 با `assertFalse("ffmpeg" in transcoder.lowercase())` روی کامنت «بدون FFmpeg» خورد؛ کامنت بازنویسی شد. قاعده: قبل از تحویل، پین‌های `assertFalse` (واژه‌های ممنوع مثل چهارگزینه، ffmpeg، innerHTML) هم در کامنت‌ها بررسی شوند.

### V135.4 — پنج گزارش کاربر پس از V135
1. **پیام کسر هزینهٔ چاپ** (`ExamHtmlPrintDialog.kt`): `barStatus` با `barStatusOk != null` اکنون ۱۲۰۰۰ms می‌ماند (خنثی ۲۶۰۰) و با `clickable` بسته می‌شود؛ متن «(برای بستن لمس کنید)».
2. **حرکت کارت‌ها** (`TeacherManagementCardsScreen.kt`): بلوکِ سوایپِ V134 حذف و بلوک V131 (شاخهٔ `direction == -1` + خروج ۳۶۰ms چپ) عیناً از `71ab527~1` برگردانده شد؛ تست‌های `V55_18*` هم به نسخهٔ پیش از V134 برگشتند.
3. **تصویر گالری در ویرایشگر/کادر متن دیده نمی‌شد**: علت ریشه‌ای — Coil 2 هر `String` را با `StringMapper` به `Uri` تبدیل می‌کند و بعد Fetcher انتخاب می‌کند؛ پس `DataUrlFetcher.Factory : Fetcher.Factory<String>` (V118) هرگز برای `AsyncImage(model = "data:…")` صدا زده نمی‌شد. `DataUrlFetcher.UriFactory : Fetcher.Factory<Uri>` (scheme=data) اضافه و در `PrivateImageLoader` ثبت شد. (استودیوی تصویر احتمالاً از مسیر دیگری کار می‌کرد؛ این Factory هر دو را پوشش می‌دهد.)
4. **برش/اندازه برای تصویر کاربر** (`AtlasEditorDialog.kt`): دکمهٔ «برش و چرخش تصویر» → data-URL به فایل موقت `cache/studio/atlas-crop-*.jpg` → `InteractiveImageEditorDialog` → خروجی دوباره `encodePhotoDataUrl` و نشانه‌ها پاک می‌شوند. لغزندهٔ «اندازهٔ نمایش» ۳۰..۱۰۰٪ → `X.w` در spec (`FigureSpec.buildAtlas(widthPercent)`, `atlasWidthPercent()`), `AtlasFigureView` با `fillMaxWidth(fraction)`. در پیش‌نمایش چاپ دستگیره‌های V129 همچنان اندازه را تعیین می‌کنند.
5. **ادغام فرمول‌های پشت‌سرهم**: `FormulaTextCodec.occurrences` توالی `$$` را نادیده می‌گیرد، پس `$a$$b$` = یک فرمول. `FormulaTextCodec.separateAdjacent()` بین «بستهٔ» یک فرمول و «بازِ» بعدی فاصله می‌گذارد؛ در `upsert` و در هر دو `onResult` ویرایشگر فرمول (`ExamBuilderScreen.kt`) اعمال می‌شود.
6. **خطای `mime type audio/mp4 is not supported`**: باکت `exam-images` فهرست MIME محدود به تصویر دارد. `SQL_NATIVE_MEDIA_COST_V135_MIME.sql` (+ migration `20260909_native_media_cost_v135_mime.sql`) نوع‌های `audio/mp4, audio/x-m4a, audio/aac, audio/mpeg` را به `allowed_mime_types` اضافه می‌کند (فقط اگر فهرست غیرخالی باشد). آپلود صوت هم `contentType = ContentType("audio","mp4")` صریح می‌فرستد.

### V135.5 — در V135.4 بلوک `cropSource?.let { InteractiveImageEditorDialog… }` با جست‌وجوی «آخرین `}` قبل از MarkingCanvas» درج شده بود و به‌اشتباه داخل `AtlasThumb` افتاد (Unresolved reference). به انتهای `AtlasEditorDialog` منتقل شد؛ لامبداهای `it` تودرتو نام‌گذاری شدند. قاعده: پس از هر درجِ برنامه‌ای، خطوط اطراف را بخوان و تعداد آکولاد را کنترل کن.

### V135.6 — ثبت‌نام مدیر: «این حساب قابل تبدیل به مدیر/معاون نیست»
- ریشه: trigger قدیمی وب‌اپ profile هر حساب ایمیلی تازه را با `role='teacher'` می‌سازد (همان مسئلهٔ V60.3). `native_complete_manager_registration_v36` (V75.1) پس از upsert، فقط `role in ('student','manager')` را می‌پذیرفت → حساب تازهٔ مدیر رد می‌شد. مسیر معلم (`native_my_registration_state_v1` V60.3) «معلمِ خالی» را می‌شناخت ولی مسیر مدیر نه.
- رفع: `supabase/migrations/20260909_native_manager_registration_v135_6.sql` (+ `sql/manual/SQL_NATIVE_MANAGER_REGISTRATION_V135_6.sql`) همان تابع V75.1 را با یک شاخهٔ اضافه بازتعریف می‌کند: اگر `role='teacher'` و بدون username/کلاس/آزمون/دانش‌آموز/عضویت فعال باشد → مجاز؛ وگرنه خطای «این حساب معلم فعال است…». گاردهای تأیید ایمیل و نقش انتخابی V75.1 دست‌نخورده‌اند. بدون تغییر Kotlin.

### V135.7 — چهار گزارش کاربر
1. **کوچک‌شدن شیء با لمس در پیش‌نمایش** (`mainscript.js`): `#pgsCanvas` با `transform: scale(z)` بزرگ‌نمایی می‌شود ولی `pointerdown` اندازهٔ `getBoundingClientRect()` (ضرب‌در z) را به‌عنوان w/h پایه می‌گرفت و اولین `pointermove` (لرزش ۱px) آن را با `setFigLayout` ذخیره می‌کرد → در زوم ۵۰٪ شکل نصف می‌شد. رفع: `zoomFactor()` (نسبت rect به offsetWidth کانواس) بر همهٔ اندازه‌ها/جابه‌جایی‌ها تقسیم شد + آستانهٔ حرکت ۳px. با Puppeteer در زوم ۰٫۵ تأیید شد (قبل: ۳۶۰→۱۸۰؛ بعد: ۳۶۰→۳۶۰).
...
### V136 — هشت مورد کاربر
1. **درج محور** — `core/figure/AxisSvgRenderer.kt` (`SUPPORTED = axnum/axxy/axq1/axgrid/axpol/ax3d`، کلیدهای X: xmin/xmax/ymin/ymax/step/title) در `FigureSvgRenderer.renderBody` وصل شد؛ `AXIS_FIGURES` در `FigureGallery.kt` (توکن با `k='g'`)؛ `FigureKind.AXIS` + پنل `GraphEditorPane` مشترک (`paramFields` برای شش نوع) در `FigurePickerDialog.kt`؛ آیکن `QuestionToolIcons.Axis`؛ `onInsertAxis` در `QuestionTextWebSection` → `ExamBuilderScreen`؛ `OptionInsertTool.AXIS`؛ در چاپ `toolOfSpec` → `"axis"` و `ExamFigureToolHost` (`NATIVE_TOOLS`). چون پیش‌نمایش چاپ شکل‌های `k='g'` را با `graph_fig.js` می‌کشد، پورت JS یک‌به‌یک همان شش نوع در `svgOf` اضافه شد (با node تست شد).
2. **تخته وایت‌برد** — `ui/student/StudentWhiteboardDialog.kt` (Dialog تمام‌صفحه، Canvas، پالت ۶ رنگ، پاک‌کن، ۳ ضخامت، برگشت، پاک‌کردن همه؛ خروجی PNG در `cache/whiteboard/` با `file:` URI). در `StudentExamScreen` بلوک `allowAnswerGraph || questionHasGraph` به دو بلوک جدا شد: `allowAnswerGraph` → `StudentWhiteboardEntry` (دکمه + پیش‌نمایش/حذف تصویر وقتی سهمیهٔ تصویر ۰ است)؛ `questionHasGraph` → همان `StudentAnswerGraph` قدیمی. `StudentExamViewModel.addResponseImages`: اگر `allowAnswerGraph` و `maxAnswerImages==0` باشد یک تصویر (جایگزین‌شونده) پذیرفته می‌شود. **پرچم JSON همان `allowAnswerGraph` ماند** (سازگاری آزمون‌های قدیمی)؛ فقط برچسب چیپ سازنده «تخته وایت‌برد دانش‌آموز» و پیش‌نمایش عوض شد.
3. **افزودن موجود** — `MemberPickerDialog(students, classes, filterMeta, schools, …)`: هدر سه‌ستونه با `weight(1f)` (برنامه سراسری RTL است؛ فرزند اول راست)، `StudentFilterDialog` با `showTeacherFilter=false` و `applyStudentFilter` مشترک؛ چیپ‌ها/ادومتر/رشته حذف شدند.
4. **اسکرول کارت** — `scrollQuestionToHeader`: اگر آیتم هدف در `visibleItemsInfo` است `animateScrollBy(offset - viewportStartOffset, tween(420))`؛ وگرنه `animateScrollToItem(target,0)` و یک اصلاح کوتاه.
5. **سوایپ چپ** — شاخهٔ `else` در `settle()` آینهٔ راست شد: `returningIndex=leaving; returnX.snapTo(x); dragX.snapTo(0); activeIndex++` سپس `returnX.animateTo(targetX,300)` و برگشت به پشته (`cards.size<=3`).
6. **تبلت دسکتاپی** — `ui/app/TabletDesktopShell.kt`: `TabletDesktopContract.usesSideRail(tablet, role)` (فقط معلم/مدیر) و `TabletSideRail` (پروفایل، دکمهٔ افزودن گرادیانی، ردیف‌های آزمون‌ها/کارت‌ها/کیف پول/داشبورد، جداکننده، همهٔ `menuCards`). در `AuthenticatedShell`: `desktopRail` → داک پایین حذف، محتوا در `Row` کنار ریل، سقف ۹۰۰dp برداشته، منوی همبرگری و کم‌رنگ‌شدن غیرفعال، `TopAppBar` می‌ماند. `featuredMenuCard` یک‌بار ساخته می‌شود. چیدمان گوشی دست نخورده است.
...
### V137 — هفت گزارش کاربر
1. **چشم اشتراک کلاس** — ریشه مثل V136 (دانش‌آموز): `native_teacher_share_class_v62` فقط UPDATE می‌زد و اگر معلم عضو هیچ مدرسه‌ای نبود یا کلاس `school_id` نداشت، مدیر (که فقط `c.school_id=مدرسهٔ خودش` را می‌بیند) چیزی نمی‌دید ولی پیام «قابل مشاهده شد» می‌آمد. `supabase/migrations/20260910_native_v137_share_class_answer_images.sql` (+ `sql/manual/SQL_NATIVE_V137_SHARE_CLASS_ANSWER_IMAGES.sql`): `native_teacher_share_class_v137` → بدون عضویت فعال خطای روشن، `school_id` خالی با مدرسهٔ معلم پر می‌شود، `shared` مؤثر برمی‌گردد. `SchoolRepository.setClassShared: Result<Boolean>`؛ `ClassesViewModel` مانند دانش‌آموز مقدار مؤثر را با خواسته مقایسه می‌کند. **باید در SQL Editor اجرا شود.**
2. **کیبورد خودکار** — `QuestionTextWebSection.kt`: `LaunchedEffect` قبلی روی `autoFocusIndex` (که به `lastTextIndex` می‌افتاد) همیشه `requestFocus` می‌زد → با بازشدن هر کارت کیبورد بالا می‌آمد. حالا `explicitFocusRequested = pendingFocusIndex != null || postInsertFocus != null` شرط است؛ یعنی فقط پس از درج/ویرایش توکن (فرمول/شکل) که خود کاربر انجام داده. هر دو سازنده (آنلاین و چاپی) از همین بخش استفاده می‌کنند.
3. **نمودار پاسخ** — چرا V136 کافی نبود: بلوک `if (questionHasGraph)` (سؤالِ دارای توکن `k='g'`) هنوز `StudentAnswerGraph` را نشان می‌داد. حالا `if (presentation.allowAnswerGraph || questionHasGraph)` فقط `StudentWhiteboardEntry` را باز می‌کند و تابع `StudentAnswerGraph` حذف شد. تست‌های V58_0_2/V58_0_3/V58_2/V59_0 به‌روز شدند (`substringBefore("private fun StudentWhiteboardEntry(")`).
4. **ترتیب آزمون‌ها** — کلاینت روی `created_at` مرتب می‌کرد ولی `native_save_exam_v1` این ستون را پر نمی‌کند و در migrationهای ریپو هیچ پیش‌فرضی برای `exams.created_at` نیست → آزمون جدید `null` و به ته فهرست می‌رفت. SQL V137: `add column if not exists created_at`, `set default now()`, پرکردن ردیف‌های خالی، تریگر `trg_native_exams_created_at_v137`. کلاینت `getMyExams` اول ردیف‌های تاریخ‌دار (نزولی) بعد بی‌تاریخ. آزمون‌های چاپی محلی از قبل با `savedAt` نزولی بودند.
5. **گزارش‌ها** — سه ریشه: (الف) `DisposableEffect(lifecycleOwner, state.finished)` پس از ارسال دوباره ساخته می‌شد و `onDispose` قدیمی با `finished=false` کهنه «exam_screen_leave» می‌نوشت؛ (ب) `ON_PAUSE` برای گالری، پنجرهٔ مجوز و خاموشی صفحه هم رخ می‌دهد → «app_leave» کاذب؛ (ج) `monitorReport()` بازهٔ سؤال جاری را با `remove` می‌بست و دوباره باز نمی‌کرد → از اولین گزارش (شروع آزمون) زمان‌سنجی سؤال متوقف می‌شد و `question_time_ms` تقریباً خالی می‌ماند. رفع: `rememberUpdatedState` برای finished/callback، فقط `ON_STOP` ثبت می‌شود و `ExamLeaveGuard.suppressNext()` قبل از بازکردن گالری آن را نادیده می‌گیرد، `recordSecurityEvent` پس از پایان/پیش از شروع بی‌اثر است؛ `monitorReport(final)` عکس لحظه‌ای بدون تغییر شمارنده‌ها می‌گیرد و `question_labels` (شمارهٔ اصلی سؤال معلم = `originalIndex+1`) و `submitted` می‌فرستد. `MonitorReportsDialog` → `Dialog(usePlatformDefaultWidth=false)` تمام‌صفحه: فهرست کارت‌های رنگی و گزارش هر دانش‌آموز (رویدادها / زمان شروع + «زمان ارسال پاسخ» یا «آخرین فعالیت» / مدت هر سؤال مرتب بر اساس شمارهٔ معلم با `faDuration`).
...
## V146 — بهینه‌سازی سایت برای گوشی و تبلت

- `site/src/site.css` لایهٔ واکنش‌گرای جدید (انتهای فایل): سه نقطهٔ شکست ۱۰۲۴ (تبلت افقی: سایدبار ۲۲۰px، ستون‌های سازنده/تصحیح باریک‌تر)، ۸۶۰ (تبلت عمودی/گوشی: سایدبار کشویی تمام‌ارتفاع با پس‌زمینهٔ تیره، هدر چسبان، مودال‌ها به‌صورت «برگهٔ پایینی»، کارت آزمون تک‌ستونه، تقویم فشرده، نوار موتور چاپ/فرمول اسکرول افقی)، ۶۰۰ (گوشی: فونت ۱۴، لندینگ فشرده، ورودی‌ها ۱۶px برای جلوگیری از زوم iOS، گزینه‌ها و جورکردنی تک‌ستونه) + `pointer:coarse` (هدف لمسی ≥۴۰px) + حالت افقی کوتاه.
- **منوی پایین گوشی** (`.bottom-nav`): ۴ مورد اول منوی نقش + «بیشتر» که سایدبار را باز می‌کند؛ روی دسکتاپ مخفی. `toggleSidebar/closeSidebar` مشترک؛ کلیک روی آیتم سایدبار آن را می‌بندد؛ `#sb-bg` پس‌زمینه.
...
## V148 — پوستهٔ موبایل پنل معلم در سایت، آینهٔ برنامهٔ اندروید (مرحلهٔ ۱: داک، منو، آزمون‌ها، کارت‌ها)

پرسش کاربر: «برای آپدیت سایت اپ هم باید آپدیت بگیرد؟» → **خیر**؛ سایت و اپ مستقل‌اند (سایت با هر push از Cloudflare Pages به‌روز می‌شود؛ اپ فقط با APK جدید). فقط وقتی قرارداد مشترک (RPC/ستون/فرمت رسانه) عوض شود هر دو باید هم‌زمان به‌روز شوند — V145 چنین بود؛ V146–V148 خیر.

### منبع حقیقت (از کد اپ، بدون حدس)
- `Design69MainMenuScreen.kt`: کارت پروفایل ۱۴۸dp، شبکهٔ ۲ستونه، کارت ۱۱۶dp، شعاع ۲۲/۲۹؛ ۸ کارت معلم به ترتیب `ExamApp.kt:947-987`: تقویم، چاپ آزمون، دانش‌آموزان، کلاس‌ها، حساب، سایت، تنظیمات، خروج (قرمز).
- `TeacherBottomDock.kt`: پنل ۸۲dp شعاع ۲۸؛ آیتم‌ها: منو، کیف پول، ⊕ وسط (افزودن سریع)، آزمون‌ها، کارت‌ها.
- `Design69QuickAddOverlay.kt`: آزمون جدید، دانش‌آموز جدید، کلاس جدید.
- `TeacherDashboardScreen.kt`: بالای فهرست راست «آزمون‌های چاپی» / وسط + / چپ «واردکردن»؛ کارت آزمون: عنوان + «باز/بسته»، خط «درس · کد · n دقیقه · بارم»؛ لمس = باز شدن ردیف آیکون‌ها: ویرایش، بستن/بازکردن، تکثیر، صادرکردن، حذف (V132).
- `TeacherManagementCardsScreen.kt`: آمار، کارنامه، بانک سؤال، تصحیح، مانده، پاسخ، درخواست‌ها با گرادیان‌های مشخص.
- رنگ‌ها `Neumorphic69Design.kt`: bg `#E9EEF5`، ink `#263142`، muted `#7B8798`، accent `#6C63F5`، accent2 `#27C4A8`، darkShadow `#ABB7C7`، danger `#C85B6B`.

### پیاده‌سازی
- `site/src/mobile.js` (جدید، آخرین فایل bundle در `build_site.py`): فعال فقط با `(max-width:860px)` و نقش معلم. `paint()` پوستهٔ `.m-shell` می‌سازد: هدر با بازگشت (به‌جز صفحه‌های آزمون‌ها/کارت‌ها/منو)، `#content`، داک، افزودن سریع، منو. صفحه‌های داخلی (کلاس‌ها، دانش‌آموزان، تصحیح، کیف پول، سازنده…) همان صفحه‌های سایت‌اند که با `S.renderPage(content)` داخل پوسته رندر می‌شوند.
- `app.js`: `render()` اول `SiteMobile.active()` را می‌پرسد؛ `renderPanel` به `renderPage(c)` شکسته شد؛ `S.render/renderPage/printExam/logout` صادر شد؛ `pageClasses` آرگومان `{create:true}` (افزودن سریع → فرم کلاس).
- `admin.js gradingPage`: `arg.filter = 'pending'|'graded'` برای کارت‌های «مانده/پاسخ».
- CSS `.m-*` با سایه‌های نئومورفیک دوطرفه (`7px 7px 16px #ABB7C7, -7px -7px 16px #fff`)، حالت فشرده inset، انیمیشن ورود stagger مثل اپ.
- کارت «سایت» در وب فقط پیام می‌دهد (کاربر همین‌جاست). «تنظیمات» → صفحهٔ ابزارها/تنظیمات سایت. «درخواست‌ها» → داشبورد (کارت درخواست‌های مدیر). «آمار/کارنامه» → گزارش‌ها.
- دسکتاپ/تبلت افقی و نقش‌های دانش‌آموز/مدیر: بدون تغییر (منوی پایین V146 برای آن‌ها می‌ماند).
- تست `V148_SiteMobileShellTest`. بدون مرورگر در سندباکس — تست بصری روی گوشی لازم است.

### مراحل بعد (V149+)
دانش‌آموز (ورود با کد، نتایج، تقویم…)، سپس مدیر؛ صفحه‌های داخلی (کلاس‌ها/دانش‌آموزان) به سبک کارت‌های اپ؛ سازندهٔ آزمون موبایل.

## V149 — دانش‌آموز در پوستهٔ موبایل + رفع «کادر خاکستری روی دسکتاپ»

### باگ دسکتاپ (V146)
- `.sb-bg` (پس‌زمینهٔ سایدبار کشویی) در V146 فقط داخل `@media (max-width:860px)` مخفی شده بود؛ روی دسکتاپ همیشه `display:block` با `position:fixed;inset:0;z-index:39` → کل صفحه خاکستری و کلیک‌ناپذیر. اصلاح: `display:none` در قاعدهٔ پایه و `.sb-bg.on{display:block}`. درس: هر عنصر overlay باید در حالت پیش‌فرض مخفی باشد، نه فقط در یک media query.

### دانش‌آموز (از کد اپ)
- `ExamApp.kt:1094,1109`: دانش‌آموز **داک پایین ندارد**؛ نوار بالا با عنوان («خانه دانش‌آموز» و…) و آیکون ☰ که منوی کاشی‌ای را باز می‌کند. ۶ کارت (`ExamApp.kt:1022+`): آزمون (ورود با کد آزمون)، نتایج من، تقویم، حساب، تنظیمات، خروج.
- `StudentHomeScreen.kt`: عنوان «داشبورد دانش‌آموز»؛ پنل «پیام جدید دارید» (cal_unseen_v59 → لمس → دیالوگ با «خواندم» → cal_mark_seen_v59)؛ پنل «کد آزمون را وارد کنید» با دکمهٔ «ورود به آزمون». در حین آزمون صفحه تمام‌صفحه است.
- پیاده‌سازی `mobile.js`: `active()` معلم + دانش‌آموز؛ `paintStudent` (هدر ☰/عنوان/خانه، بدون داک)؛ `studentMenu()`؛ `studentHome()` = عنوان + پنل پیام‌ها + `SiteStudent.page()` داخل پنل نئومورفیک (`.m-join`). `student.js startExam`: در پوستهٔ موبایل ابتدا `paint()` (هدر حذف، `inExam` true) و آزمون در `#content` تمام‌صفحه رندر می‌شود؛ پایان آزمون با `S.go('grades')` پوسته را برمی‌گرداند.
- تست `V149_SiteMobileStudentTest`. مدیر/معاون هنوز پوستهٔ دسکتاپی/منوی پایین V146 را دارد (V150).

## V150 — مدیر/معاون در پوستهٔ موبایل (هر سه نقش کامل)

از کد اپ: `ExamApp.kt:988-1019` منوی مدیر ۶ کارت (کلاس‌ها، دانش‌آموزان، حساب، سایت، تنظیمات، خروج) بدون تقویم/چاپ + کارت ویژهٔ «داشبورد / اطلاعات مدرسه و آمار» وسط‌چین با عرض ۵۲٪ (`Design69MainMenuScreen featuredCard`)؛ داک مدیر: آیتم اصلی «معلم‌ها» با آیکون Students (`ExamApp.kt:1136`)؛ افزودن سریع مدیر: اصلی «دعوت معلم» + دانش‌آموز/کلاس/مدرسه جدید (`Design69QuickAddOverlay` ACTION_COUNT=4, `ExamApp.kt:1191`)؛ کارت‌های مدیریتی مدیر: مدارس، کارنامه، وضعیت (`TeacherManagementCardsScreen.kt:165-195`).
- `mobile.js`: `active()` هر سه نقش؛ `isManager()`؛ `managerMenu()`، `managerCards()`؛ داک/افزودن سریع نقش‌محور؛ `MANAGER_TITLES`؛ خانهٔ مدیر = «معلم‌ها» (صفحهٔ سایت با دعوت/انتقال اعتبار). «دانش‌آموزان» → `school {students:true}` (اسکرول به بخش دانش‌آموزان مدرسه، `admin.js`). معلم: «مدرسه جدید» → صفحهٔ حساب (کارت عضویت با کد دعوت `joinSchoolCard`).
- افزودن سریع معلم هم اکنون ۴ عمل دارد (مثل اپ).
- تست `V150_SiteMobileManagerTest`. بعدی: صفحه‌های داخلی (کلاس‌ها/دانش‌آموزان/کیف پول) به سبک کارت‌های اپ به‌جای جدول؛ سازندهٔ آزمون موبایل.

## V151 — صفحه‌های داخلی پوستهٔ موبایل به سبک اپ (جدول → کارت)

- مرجع اپ: `SchoolManagementScreen.kt:737-775` کلاس = Card با عنوان + «پایه: … · رشته»، «اعضا: n نفر · پسر · دختر»، ردیف عملیات وسط‌چین؛ دانش‌آموز (`:1320-1440`) = کارت با نام، نام پدر، نام کاربری، کلاس‌ها و آیکون‌های عملیات.
- سایت: همهٔ صفحه‌های داخلی (کلاس‌ها، دانش‌آموزان، roster، معلم‌ها، کیف پول، تصحیح، آزمون‌های چاپی، بانک…) با `table.tbl` ساخته می‌شوند (۲۱ نقطه). به‌جای بازنویسی هر صفحه، `mobile.js tableToCards()` هر جدول را در پوستهٔ موبایل به `.m-rows > .m-rowcard.neo` تبدیل می‌کند: ستون اول = عنوان کارت، ستون‌های میانی = «سرستون: مقدار»، ستون عملیات (icon-btn/btn) = ردیف پایین وسط‌چین. **گره‌های DOM اصلی منتقل می‌شوند** (نه کپی) → onclickها و رفتار دست‌نخورده. جدول اصلی `display:none` و `data-m-cards` می‌گیرد (idempotent). MutationObserver مستقل از `wrapTables` V146.
- CSS `.m-mode .m-content …`: icon-btn/دکمه/ورودی/تب/کارت آمار/کیف پول/مودال‌ها با سایه‌های نئومورفیک اپ؛ نوار ابزار بالای صفحه: دکمهٔ اصلی پهن (`weight 2`) و ثانویه (`weight 1`) مثل «ساخت کلاس جدید / مدارس» اپ.
- فقط در `active()`؛ دسکتاپ بدون تغییر. تست `V151_SiteMobileInnerPagesTest`. بعدی: سازندهٔ آزمون در گوشی (ExamBuilderScreen: کارت‌های سؤال، منوی شعاعی ابزار).

## V152 — سازندهٔ آزمون در گوشی به سبک اپ + توقف موقت CI اپ

### CI اپ (درخواست کاربر)
- `android.yml`: trigger `push` کامنت شد؛ فقط `workflow_dispatch` (GitHub → Actions → «ساخت APK…» → Run workflow). برای بازگشت، سه خط push را از کامنت درآورید؛ پیشنهاد: با `paths-ignore: ['site/**','docs/**','text/**']` تا تغییرات سایت بیلد اپ را راه نیندازد. توجه: تست‌های آینهٔ سایت (V139…V152) در CI اپ‌اند و تا اجرای دستی بعدی سبز/قرمز بودنشان معلوم نمی‌شود — رشته‌ها با grep تأیید شده‌اند.
- `site.yml` دست‌نخورده (سایت با هر push منتشر می‌شود).

### سازندهٔ آزمون موبایل (از `ExamBuilderScreen.kt`)
- اپ: TopAppBar «ساخت آزمون/ویرایش آزمون» + بازگشت؛ `OutlinedButton fillMaxWidth` «مشخصات آزمون»؛ FAB سبز `#27A86B` ✓ ذخیره (CenterStart) و FAB «+» (CenterEnd) → `BuilderRadialMenuOverlay` با ۸ عمل و رنگ‌های `QuestionDraft.pastelColor` (تشریحی FFD1DC، چندگزینه‌ای AEC6CF، صحیح/غلط B4EEB4، جای خالی FDFD96، عددی C3B1E1، جورکردنی FFDAB9، وارد کردن 98FF98، بانک E6E6FA)؛ حالت چاپ FAB سوم پیش‌نمایش/چاپ.
- سایت `mobile.js builderFabs(c)`: پس از ساخت `.b-top` توسط builder.js (MutationObserver)، دکمه‌های نوار بالا مخفی و به FABهای ثابت (`.m-bfab`، بالای داک) نگاشت می‌شوند: ذخیره → کلیک دکمهٔ ذخیرهٔ اصلی؛ + → منوی شعاعی (دو حلقهٔ ربع‌دایره ۴+۴ در گوشهٔ راست‌پایین)؛ هر نوع → کلیک دکمهٔ همان نوع در `.b-add` (پنهان در موبایل) و اسکرول به ویرایشگر؛ وارد کردن → `SiteExtras.importExam`؛ بانک → دکمهٔ «از بانک سؤال». «مشخصات آزمون» تمام‌عرض زیر فیلدها. فهرست سؤال‌ها به‌صورت تراشه‌های افقی اسکرول‌شونده بالای ویرایشگر؛ ابزارها/ورودی‌ها نئومورفیک.
- منطق builder.js تغییری نکرد (فقط DOM موجود هدایت می‌شود) → دسکتاپ و ذخیره/آپلود دست‌نخورده.
- تست `V152_SiteMobileBuilderTest`.

## V153 — مقایسهٔ کامل اپ با سایت گوشی و هم‌شکل‌سازی

روش: هر صفحهٔ پوستهٔ موبایل با Playwright (Chromium headless، ۳۹۰×۸۴۴، fetch شبیه‌سازی‌شده، `ExamSite.__setUser`) اسکرین‌شات شد و با کد Compose مقایسه شد. یکسان بود: داک، منوی کاشی‌ای معلم/مدیر/دانش‌آموز (متن‌ها و ترتیب عیناً ExamApp.kt:949–1047)، آزمون‌ها (TeacherDashboardScreen: ردیف بالا، کارت باز/بسته، آیکن‌های V132)، خانهٔ دانش‌آموز، معلم‌های مدیر. تفاوت‌ها و اصلاح:
1. **افزودن سریع** — اپ (`Design69QuickAddOverlay`): پنل فرورفتهٔ ۴۶۶dp با گوشهٔ ۴۸، چهار کارت ۸۸dp (گوشهٔ ۲۹) در چهار گوشهٔ ضربدر (افقی ۲۹٪ عرض ۹۴..۱۲۴، عمودی ۱۰۸)، خط‌چین از مرکز، دکمهٔ + به مرکز می‌آید و ۱۳۵° می‌چرخد. سایت قبلاً لیست پایین‌صفحه بود → `quickAdd()` با `.m-qa-*` بازسازی شد.
2. **کارت‌ها** — اپ (`TeacherManagementCardsScreen`): دستهٔ ۳ کارت روی هم (۹۰٪×۱۹۰dp، گوشهٔ ۲۹، کارت‌های پشتی ۵° و −۶°)، کشیدن افقی >۵۲dp کارت را پرتاب می‌کند، نقطه‌ها (۲۳/۷)، پنل توضیح فرورفته، ضربهٔ دوبارهٔ داک روی «کارت‌ها» = کارت بعدی (`cardsCycleKey`). سایت قبلاً فهرست عمودی بود → `cardsDeck()`؛ گرادیان‌های مانده/پاسخ/درخواست‌ها هم مطابق اپ شد (E0587F→7D6CF4، 4D5B74→273247، 7D6CF4→E0587F). مدیر هم همین دسته (۳ کارت).
3. **منوی شعاعی سازنده** — اپ: دایرهٔ کامل وسط صفحه، شعاع ۳۱٪ عرض (۱۰۴..۱۳۸)، حلقهٔ خط‌چین، ۸ مربع گوشه‌گرد ۶۶dp از ساعت ۱۲ هر ۴۵°، دکمهٔ ✕ گرادیانی→قرمز در مرکز، FAB + پنهان تا بسته شود. V152 ربع‌دایرهٔ گوشه بود → اصلاح شد.
...
## V155 — بدون زوم/اسکرول اضافی + چاپ آزمون و ایجاد آزمون مثل اپ

- **زوم/اسکرول:** `viewport` با `maximum-scale=1.0, user-scalable=no`؛ در `.m-mode` `overscroll-behavior:none` و `touch-action:pan-x pan-y` (بدون pinch)؛ ورودی‌ها ۱۶px تا iOS هنگام فوکوس زوم نکند؛ پدینگ پایین محتوا کم شد تا وقتی محتوا کوتاه است اسکرول نشود.
...
## §V158 — دکمهٔ برگشت گوشی + «افزودن سریع» مثل اپ
- `site/src/mobile.js`: `pushHist()/onBack()` روی `popstate` (آینهٔ `BackHandler` در `ExamApp.kt:358–364`): ترتیب = بستن بالاترین overlay (`.m-sheet-bg/.modal-bg/.engine-bg/.m-radial-bg/.m-qa-bg`) → بستن منو/افزودن سریع → سازندهٔ چاپی به «چاپ آزمون» → صفحهٔ خانهٔ نقش (`homePanel()`) → در خانه `confirmDlg('خروج','از سایت خارج می‌شوید؟')` و با تأیید `S.logout()` + `history.go(-n)`. `S.go` در حالت گوشی wrap شده تا هر ناوبری یک ورودی history بسازد. داخل آزمون دانش‌آموز، برگشت فقط پیام می‌دهد.
- افزودن سریع (`quickAdd`, آینهٔ `createStudent/createExam/createSchool` در ExamApp.kt:249–315): «دانش‌آموز جدید» → معلم: `bulkDialog` (BulkStudentDialog)؛ مدیر: `managerStudentPicker()` («انتخاب معلم و کلاس»: `native_manager_teachers_v37` → `native_manager_teacher_classes_v40c`) سپس `bulkDialog(..., afterCreate)` که با `native_manager_set_class_student_v40c` به کلاس معلم اضافه می‌کند. «آزمون جدید» → `go('builder', null)` (آنلاین). «مدرسه جدید» → معلم `joinSchoolDialog()` (`native_join_school_v39`)، مدیر `createSchoolDialog()` (`native_manager_create_school_v61`).
- تست: `V158_SiteMobileBackQuickAddTest.kt`.
- **V158.1 (علت باگ «آزمون جدید → سازندهٔ چاپی»)**: `builder.page()` پیش‌نویس `examsite.builderdraft.v1` را بدون توجه به `mode` برمی‌گرداند؛ اگر آخرین کار کاربر یک آزمون چاپی ناتمام بود، `state = draft` با `mode:'print'` می‌شد. اصلاح: پیش‌نویس فقط وقتی `draft.mode !== 'print' && !draft.bankEdit`. (پیش‌نویس چاپی همچنان از «آزمون‌های چاپی» قابل ادامه است.)

## §V159 — مشخصات آزمون در پنجره + استودیوی تصویر سایت
- `site/src/builder.js`: نوار بالا (`b-top`) دیگر ورودی عنوان/درس/مدت ندارد. آنلاین: این سه فیلد در `openSettings` (آینهٔ `ExamSettingsCard` در ExamBuilderScreen.kt:767) و خلاصه در `.b-meta`. چاپی: بدون فیلد (مثل `printMode` اپ که فقط «تنظیمات سربرگ» دارد)؛ ذخیره با `S.promptDlg('ذخیره آزمون چاپی', …)` نام را می‌پرسد (آینهٔ `askPrintName` خط 627). اعتبارسنجی عنوان فقط برای آنلاین.
- `site/src/app.js`: `promptDlg(title, body, label, value, okLabel)` → Promise<string|null>.
- `site/src/studio.js` (جدید، در `build_site.py` بعد از builder.js): `window.SiteStudio.open({existing, onInsert, onReplace, onDelete, onSplitToSame, onSplitToQuestions})` — آینهٔ `ExamImageStudioDialog` (ExamImageStudioCore.kt). حالت: `st.orig` (canvas ≤2200px) → `base()` = چرخش+صاف‌سازی+قرینه (کش) → `render()` = برش → شکل‌ها → deshadow/threshold/despeckle → outSize → JPEG با `quality` (همان زنجیرهٔ `processAndEncode/encodeCropped`). شکل‌ها با مختصات نرمال (0..1) نسبت به تصویر پایه؛ تب‌ها image/draw/enhance/deskew. خروجی همیشه dataURL؛ در مسیر آنلاین همان حلقهٔ قبلی ذخیره (`/^data:/` → `uploadImage`) آن را آپلود می‌کند.
...
## §V160.2 — ریشهٔ 403 آروان: ACL شیء
خروجی عیب‌یابی V160.1: `azmoon-media.s3.ir-thr-at1.arvanstorage.ir — HTTP 403` در حالی که صندوقچه عمومی است و فایل از پنل باز می‌شود (پنل با کلید امضا می‌کند). در S3 آروان دسترسی عمومیِ صندوقچه به اشیاء جدید ارث نمی‌رسد؛ هر PUT بدون `x-amz-acl: public-read` شیء خصوصی می‌سازد. رفع: `media-upload/index.ts` هدر `x-amz-acl` را در SignedHeaders/CanonicalHeaders امضا و در `headers` پاسخ برمی‌گرداند؛ `app.js uploadMedia` همهٔ `t.headers` را می‌فرستد؛ `SupabaseQuestionImageUploader.uploadBytes` هدرهای غیر Content-Type پاسخ را اضافه می‌کند. **باید Edge Function دوباره deploy شود** (`supabase functions deploy media-upload`). اشیاء قدیمی: `scripts/arvan_make_public.py` (boto3، کلیدها فقط در env). CORS صندوقچه باید هدر `x-amz-acl` را هم مجاز کند (Allowed Header `*` کاربر کافی است).
...
## §V160.5 — قاعدهٔ دائمی CI اپ (دستور کاربر)
- نسخه‌ای که **کد اپ** (`app/**`, gradle, …) را تغییر می‌دهد → در `.github/workflows/android.yml` سه خط `push:` باید **باز** باشد تا APK ساخته شود.
...
## §V162 — سرعت سایت (جداسازی موتورها) + «تنظیمات» گوشی
- **علت کندی:** `site/index.html` ۶٫۳ مگابایت بود چون `build_site.py` موتور چاپ (assets/print، شامل math_host 1.5MB و قلم‌ها) و `formula.html` (2MB) را داخل `window.__ENGINES` تعبیه می‌کرد؛ هر بازکردن سایت = دانلود + پارس ۶ مگابایت (Cache-Control: no-cache برای index).
- **راه‌حل:** `build_site.py` موتورها را در `site/engines.<sha256[:12]>.js` می‌نویسد (فایل‌های engines قدیمی را پاک می‌کند) و در index فقط `window.__ENGINES_URL` می‌گذارد. `app.js`: `loadEngines()` (تزریق `<script>` تنبل، یک‌بار)، `engineHtml(kind)` حالا **Promise** برمی‌گرداند؛ همهٔ مصرف‌کننده‌ها (`openPrintPreview`، `openFormulaEditor`، builder `ensurePreviewFrame`/`insertFigure`، student `ensureMathFrame`) به `.then` تبدیل شدند. `prefetchEngines()` بعد از ورود/بازیابی نشست در زمان بیکاری صفحه موتور را پیش‌بارگذاری می‌کند.
- **استقرار:** `site.yml` فایل engines را در `site_out/` کپی و `_headers` برای `/engines.*.js` مقدار `max-age=31536000, immutable` می‌گذارد؛ `sw.js` آن را کش-اول نگه می‌دارد و نسخه‌های قدیمی موتور را پاک می‌کند. سایت دیگر «تک‌فایلی محض» نیست (کاربر در V162 موافقت کرد) — index.html ≈ ۰٫۷ مگابایت.
...
## V165 — پوستهٔ دسکتاپ سایت به سبک «نسخهٔ یکپارچه» (ریل عمودی + صفحهٔ منو)

- **مرجع:** فایل قدیمی `نسخه یکپارچه 9.79M` (body.final-compact): `.bottom-dock` در دسکتاپ ریل عمودی سمت راست بود (۷۲px، هالهٔ نئونی #22c6ef روی فعال، نام هنگام hover)؛ منوی همبرگری = کارت پروفایل + `.menu-grid` چهارستونه.
- **app.js:** `renderPanel()` روی `body.dk` می‌گذارد و `rail` (`.dk-rail` با `--n` = تعداد موارد) را کنار سایدبار/منوی پایین قدیمی به `.app` اضافه می‌کند؛ سایدبار و bottom-nav فقط برای ≤ 860px می‌مانند (CSS آن‌ها را روی دسکتاپ پنهان می‌کند). `pageMenu(c)` صفحهٔ «منو» (view.panel === 'menu') است. آیکون‌ها در `DK_ICONS`، توضیح کارت‌ها در `DK_SUBS`، `dkGo(panel)` ناوبری. `render()` کلاس `dk` را پاک می‌کند تا لندینگ/گوشی تحت تأثیر نباشند.
- **site.css:** بلوک V165 در انتها؛ همهٔ قواعد زیر `.dk` و `@media (min-width:861px)`؛ پالت `body.dk` متغیرهای `--brand/--bg/...` را بازتعریف می‌کند تا کارت‌ها/دکمه‌ها/فرم‌های موجود بدون تغییر کد رنگ جدید بگیرند. `.dk .app{overflow-x:clip}` چون ریل fixed خارج از جریان است.
- **نکته:** برای پیش‌نمایش محلی بدون سرور، `index.html` را با jsdom و fetch جعلی رندر و با Playwright اسکرین‌شات گرفتیم (ابزار موقت؛ در مخزن نیست).
- تست: `V165_SiteDesktopRailTest`.

## V166 — سازندهٔ آزمون دسکتاپ به سبک «نسخهٔ یکپارچه»
...
## §V180.2 — صفحهٔ خالی پس از چاپ در سایت

**گزارش:** پس از «کسر ۱٬۰۰۰ تومان با موفقیت» فقط نوار «پیش‌نمایش و چاپ — آزمون» و پس‌زمینهٔ خاکستری (#e8ecf1 body موتور) می‌ماند.

**علت:** در مسیر چاپ مستقیم (`opts.printMode` از منوی «چاپ آزمون/چاپ با کلید» بیلدر) `setExamData → printStudent()` بدون
`showPreview` اجرا می‌شود؛ بینندهٔ PGS هرگز باز نشده و پس از پنجرهٔ چاپ `restorePreview` فقط `renderPreview` می‌کند (که در میزبان
پنهان است). اپ در همین حالت `if (initialPrintMode != null) requestDismiss()` می‌کند. رفع: `printCtx.direct`؛ پس از چاپ مستقیم
...
## §V199 — پرداخت هزینهٔ چاپ آزمون چاپی
- **سرور** (`supabase/migrations/20260925_native_print_pay_v199.sql` = `sql/manual/SQL_NATIVE_PRINT_PAY_V199.sql`): جدول `print_exam_payments(print_exam_id pk, header_hash, q_hashes {کلید سؤال: [URL تصاویر]}, paid_total)`؛ `native_print_q_key_v199` = md5(سؤال بدون فیلدهای تصویر)، `native_print_q_images_v199` = URLهای http (رشته یا {uri})؛ `native_print_header_hash_v199(exam, p_header)` = md5(p_header|subject|duration)؛ `native_print_due_v199` بدهی (سربرگ متفاوت/بدون سابقه → همه؛ وگرنه سؤال‌های با کلید جدید + تصاویر پرداخت‌نشده در کل آزمون)؛ RPCها: `native_print_quote_v199(p_id,p_header)`, `native_print_pay_v199(p_id,p_operation,p_header)` (idempotent، wallet_tx reason `print_exam:print:<id>`)، `native_print_pay_status_v199(p_header)` → [{id,due,paid}]. `native_print_exam_save_v163` بازنویسی شد با `v_cost := 0` (ذخیره رایگان).
- **اثر انگشت سربرگ** (کلاینت): فیلدهای `f_*` به‌جز `f_course/f_duration`، خالی‌ها حذف، `f_headerTemplate` خالی = classic، مرتب، «k=v» با `\n`. اپ: `ui/printing/PrintPayFingerprint.kt`؛ سایت: `headerFingerprint()`/`printHeaderFp()` در app.js. هر دو باید یکسان بمانند (تست V199).
- **سایت**: `api.printQuote/printPay/printPayStatus`؛ `ensurePrintPaid(printId, fp, title)` (برآورد→تأیید→کسر)؛ پل چاپ: اگر `ctx.printExam !== undefined` مسیر جدید (بدون ذخیره/با dirty چاپ ممنوع؛ پس از پرداخت `ctx.paid` هر دو نسخه)؛ `setPreviewPaid/refreshPreviewPaid` → `w.setPrintPaid(bool)` در webhost.js → کلاس `print-paid` روی body (pgs_style.css: قرمز/سبز `.pgs-btn.primary`). کارت‌ها: builder.js `printExamsSection` (PAY_ICON/PRINT_ICON، کلاس paid/unpaid) و mobile.js `printCenter` (act wallet/print). ذخیرهٔ چاپی بدون پرسش هزینه.
- **اپ**: `SupabasePrintExamRepository.quote/pay/payStatus`؛ `ExamPrintCenterScreen` آیکون کیف پول (PrintPayDialog) و چاپگر قرمز/سبز (لمس → آزمون‌ساز چاپی)؛ `ExamHtmlPrintDialog(printPayExamId, printPayDirty)`؛ `ExamBuilderViewModel.hasUnsavedChanges()` (cleanDraftFingerprint؛ `applyPrintSaved` آن را تازه می‌کند). مسیر قدیمی V132 (`native_charge_print_v1`) برای چاپ آزمون آنلاین دست‌نخورده است.
