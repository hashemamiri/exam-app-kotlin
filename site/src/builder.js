...
(function () {
  ...
  /* کادر متن با روکش تراشه‌ها: در حالت عادی روکش دیده می‌شود؛ با کلیک/فوکوس، متن خام برای ویرایش نمایان است */
  function tokenTextarea(ta) {
    var wrap = el('div', {class: 'b-ta-wrap'}); var mask = el('div', {class: 'b-ta-mask'});
    var live = el('div', {class: 'b-live', style: 'display:none'});
    previewCss();
    function sync() { mask.innerHTML = maskedHtml(ta.value) || '<span class="ph">' + esc(ta.placeholder || '') + '</span>'; wrap.classList.toggle('has-token', /\$[^$]+\$|%%FIG:/.test(ta.value)); livePreview(live, ta.value); }
    ta.addEventListener('input', sync); ta.addEventListener('focus', function () { wrap.classList.add('editing'); }); ta.addEventListener('blur', function () { wrap.classList.remove('editing'); sync(); });
    mask.addEventListener('click', function () { ta.focus(); });
    wrap.appendChild(ta); wrap.appendChild(mask); sync();
    return {wrap: wrap, live: live, sync: sync};
  }
